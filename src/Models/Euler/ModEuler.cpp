...
ModEuler::ModEuler(const int& numbTransports) : Model(NAME, numbTransports)
{
...
void ModEuler::allocateCons(Flux** cons) { *cons = new FluxEuler; }

//***********************************************************************

void ModEuler::allocatePhase(Phase** phase) { *phase = new PhaseEuler; }

//***********************************************************************

void ModEuler::allocateMixture(Mixture** mixture) { *mixture = new MixEuler; }

//***********************************************************************

void ModEuler::allocatePhaseGradient(GradPhase** phase) { *phase = new GradPhaseEuler; }

//***********************************************************************

void ModEuler::allocateMixtureGradient(GradMixture** mixture) { *mixture = new GradMixEuler; }

//***********************************************************************

void ModEuler::fulfillState(Phase** phases, Mixture* /*mixture*/) { phases[0]->extendedCalculusPhase(phases[0]->getVelocity()); }

//****************************************************************************
//********************* Cell to cell Riemann solvers *************************
//****************************************************************************

void ModEuler::solveRiemannIntern(
...
void ModEuler::solveRiemannInternMRF(Cell& cellLeft,
                                     Cell& cellRight,
                                     const double& dxLeft,
...
void ModEuler::solveRiemannInletTank(Cell& cellLeft,
                                     const double& dxLeft,
                                     double& dtMax,
                                     const double* /*ak0*/,
                                     const double* rhok0,
...
void ModEuler::solveRiemannInletInjStagState(Cell& cellLeft,
                                             const double& dxLeft,
                                             double& dtMax,
                                             const double m0,
                                             const double* /*ak0*/,
...
void ModEuler::solveRiemannInletInjTemp(
  Cell& cellLeft, const double& dxLeft, double& dtMax, const double m0, const double* Tk0, const double* /*ak0*/, std::vector<double>& boundData) const
{
  ...
  do {
    ...
    it++;
    if (it > 50) {
      Errors::errorMessage("solveRiemannInletInjTemp not converged in modEuler");
    }
    eos->verifyAndModifyPressure(pStar);

    // Left intermediate state
    rhoStarL = eos->computeDensityIsentropic(pL, rhoL, pStar, &drhoStarL);
    // rhoStarL = eos->computeDensityHugoniot(pL, rhoL, pStar, &drhoStarL);
    //AF//TODO// use vStarL instead of 1/(rho*rho)?
    vStarL  = 1. / rhoStarL;
...
void ModEuler::solveRiemannTransportIntern(Cell& cellLeft, Cell& cellRight)
{
  for (int k = 0; k < numberTransports; k++) {
    fluxBufferTransport[k].solveRiemann(cellLeft.getTransport(k).getValue(),
                                        ...
                                        static_cast<FluxEuler*>(fluxBuff)->m_sM);
  }
}
...
{
  for (int k = 0; k < numberTransports; k++) {
    fluxBufferTransport[k].solveRiemannOutletPressure(cellLeft.getTransport(k).getValue(),
                                                      static_cast<FluxEuler*>(fluxBuff)->m_sM,
                                                      valueTransports[k]);
  }
}
...
const double& ModEuler::getSM() { return static_cast<FluxEuler*>(fluxBuff)->m_sM; }
...
void ModEuler::lowMachSoundSpeed(double& machRef, const double& uL, double& cL, const double& uR, double& cR) const
{
  ...
  double machRefL(0.);
  //AF//TODO// std::fabs(uL) >= cL * machLimitComp
...
void ModEuler::addNonConsMrfFlux(Phase** phases) { static_cast<FluxEuler*>(fluxBuffMRF)->addNonConsMrfFlux(phases); }
...
{
  Coord projectedFlux;
  projectedFlux.setX(normal.getX() * static_cast<FluxEuler*>(fluxBuffMRF)->m_momentum.getX() +
                     tangent.getX() * static_cast<FluxEuler*>(fluxBuffMRF)->m_momentum.getY() +
                     binormal.getX() * static_cast<FluxEuler*>(fluxBuffMRF)->m_momentum.getZ());
  projectedFlux.setY(normal.getY() * static_cast<FluxEuler*>(fluxBuffMRF)->m_momentum.getX() +
                     tangent.getY() * static_cast<FluxEuler*>(fluxBuffMRF)->m_momentum.getY() +
                     binormal.getY() * static_cast<FluxEuler*>(fluxBuffMRF)->m_momentum.getZ());
  projectedFlux.setZ(normal.getZ() * static_cast<FluxEuler*>(fluxBuffMRF)->m_momentum.getX() +
                     tangent.getZ() * static_cast<FluxEuler*>(fluxBuffMRF)->m_momentum.getY() +
                     binormal.getZ() * static_cast<FluxEuler*>(fluxBuffMRF)->m_momentum.getZ());
