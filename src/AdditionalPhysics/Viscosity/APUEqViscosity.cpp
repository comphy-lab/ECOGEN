...
void APUEqViscosity::addQuantityAddPhys(Cell* cell) { cell->getVecQuantitiesAddPhys().push_back(new QAPViscosity(this)); }
...
void APUEqViscosity::solveFluxAddPhys(CellInterface* cellInterface)
{
  ...
  this->solveFluxViscosityInner(
    m_velocityLeft, m_velocityRight, m_gradULeft, m_gradURight, m_gradVLeft, m_gradVRight, m_gradWLeft, m_gradWRight, muMixLeft, muMixRight);
                                             ...
                                             const double& muMixRight) const
{
  ...
  double dwdx, dwdz;
  double muMel;
  //AF//TODO precompute one_half, one_third...
  u = (uL + uR) / 2.;
  v = (vL + vR) / 2.;
  w = (wL + wR) / 2.;
...
void APUEqViscosity::solveFluxViscosityNonReflecting(
  const Coord& velocityLeft, const Coord& gradULeft, const Coord& gradVLeft, const Coord& gradWLeft, const double& muMixLeft) const
{
  this->solveFluxViscosityInner(velocityLeft, velocityLeft, gradULeft, gradULeft, gradVLeft, gradVLeft, gradWLeft, gradWLeft, muMixLeft, muMixLeft);
}
...
void APUEqViscosity::addNonCons(Cell* cell)
{
  ...
  double termeNonCons = -2. / 3. * (dudx + dvdy + dwdz) * (dudx + dvdy + dwdz) + 2. * (dudx * dudx + dvdy * dvdy + dwdz * dwdz) +
