...
class EosNASG : public Eos
{
  public:
    ...
    double computeTemperature(const double& density, const double& pressure) const override;
    ...
    double computeEnergy(const double& density, const double& pressure) const override;
    ...
    double computePressure(const double& density, const double& energy) const override;
    ...
    double computeDensity(const double& pressure, const double& temperature) const override;
    ...
    double computeSoundSpeed(const double& density, const double& pressure) const override;

    //! \brief     Compute interface sound speed
    //! \param     density            density (\f$\rho\f$)
    //! \param     interfacePressure  interface pressure (pI)
    //! \param     pressure           pressure (p)
    //! \return    soundSpeed
    //! \details   with soundSpeed : \f$  c(p, \rho)  = \sqrt{  \frac{(\gamma-1) \ p_I + p + \gamma \ p_{\infty}}{(1 - \rho \ b) \ \rho}} \f$
    double computeInterfaceSoundSpeed(const double& density, const double& interfacePressure, const double& pressure) const override;

    //! \brief     Compute acoustic impedance
    //! \param     density            density (\f$\rho\f$)
    //! \param     pressure           pressure (p)
    //! \return    acousticImpedance
    //! \details   with : \f$  z(p, \rho)  = \sqrt{ \frac{\rho \ \gamma \ (p+p_{\infty})}{1-\rho b} } \f$
    double computeAcousticImpedance(const double& density, const double& pressure) const override;

    //! \brief     Compute density times interface sound speed square
    //! \param     density            density (\f$\rho\f$)
    //! \param     interfacePressure  interface pressure (pI)
    //! \param     pressure           pressure (p)
    //! \return    rhoCISquare
    //! \details   with : \f$  z(p, \rho)  = \frac{(\gamma-1) \ p_I + p + \gamma \ p_{\infty}}{1 - \rho \ b} \f$
    double computeDensityTimesInterfaceSoundSpeedSquare(const double& density, const double& interfacePressure, const double& pressure) const override;
    ...
    double computeEntropy(const double& temperature, const double& pressure) const override;
    ...
    double computePressureIsentropic(const double& initialPressure, const double& initialDensity, const double& finalDensity) const override;
    ...
    double computePressureHugoniot(const double& initialPressure, const double& initialDensity, const double& finalDensity) const override;
    ...
    //! \details    with finalDensity : \f$ \rho_f = \rho_i \frac{(p_f + p_{\infty})^{1/\gamma}}{(p_i + p_{\infty})^{1/\gamma} (1-\rho_i b) + \rho_i b (p_f + p_{\infty})^{1/\gamma}} \f$ AND with   *drhodp : \f$ \left. \frac{\partial \rho}{\partial p} \right)_s = \frac{\rho_f (1-\rho_f b)}{\gamma (p_f+p_{\infty})} \f$
    double computeDensityIsentropic(const double& initialPressure,
                                    const double& initialDensity,
                                    const double& finalPressure,
                                    double* drhodp = 0) const override;
    ...
    //! \param     finalPressure      final pressure (\f$ p_f \f$)
    //! \return    finalDensity  AND  *drhodp (OPTIONAL)
    //! \details    with finalDensity : \f$ \rho_f =\rho_i\frac{ (\gamma + 1)*(p_f+p_{\infty}) + (\gamma - 1)*(p_i+p_{\infty})}{(\gamma - 1)(p_f+p_{\infty}) +(\gamma + 1)(p_i+p_{\infty}) + 2 b \rho_0 (p_f-p_i)} \f$ AND with  *drhodp :  \f$  \left.  \frac{\partial \rho}{\partial p} \right)_{Hug} = \frac{4\gamma \rho_i \ (p_i+p_{\infty}) \ (1- b\rho_i)}{ \left( (\gamma - 1)(p_f+p_{\infty}) +(\gamma + 1)(p_i+p_{\infty}) +2b\rho_i (p_f-p_i)\right) ^2} \f$
    double computeDensityHugoniot(const double& initialPressure,
                                  const double& initialDensity,
                                  const double& finalPressure,
                                  double* drhodp = 0) const override;
    ...
    //! \param     finalPressure      final pressure (\f$ p_f \f$)
    //! \return    finalDensity  AND  *drhodp (OPTIONAL)
    //! \details    with finalDensity : \f$ \rho_f = \rho_i \frac{\gamma (p_f+p_{\infty})}{\gamma (p_f+p_{\infty}) + p_i - p_f + b \rho_i (p_f -p_i)} \f$ AND with *drhodp : \f$ \frac{d\rho}{dp} = \rho_i \gamma \frac{\left(p_i + p_{\infty}-b \rho_i (p_i + p_{\infty})\right)}{\left(\gamma (p_f+p_{\infty}) + p_i - p_f+b \rho_i (p_f - p_i)\right)^2} \f$
    double
    computeDensityPfinal(const double& initialPressure, const double& initialDensity, const double& finalPressure, double* drhodp = 0) const override;
    ...
    //! \details   with  rhoe : \f$  \rho\epsilon(p, \rho) = \frac{(p+\gamma p_{\infty})(1-\rho b)}{(\gamma-1)} +\rho\epsilon_{ref} \f$ AND \f$ \frac{d \rho\epsilon}{d p} =\left.  \frac{\partial \rho\epsilon}{\partial p} \right)_{\rho} + \left.  \frac{\partial \rho\epsilon}{\partial\rho  } \right)_p \frac{d\rho}{dp} = \frac{1-\rho b}{(\gamma-1)} +\frac{d \rho}{d p}\left(\epsilon_{ref}-b\frac{(p+\gamma p_{\infty})}{\gamma-1}\right) \f$
    double computeDensityEnergySaturation(const double& pressure, const double& rho, const double& drhodp, double* drhoedp = 0) const override;
    ...
    void sendSpecialMixtureEos(double& gamPinfOverGamMinusOne, double& eRef, double& oneOverGamMinusOne, double& covolume) const override;
    ...
    double vfpfh(const double& pressure, const double& enthalpy) const override;
    ...
    double dvdpch(const double& pressure, const double& enthalpy) const override;
    ...
    double dvdhcp(const double& pressure) const override;

    //! \brief  Compute the partial derivative of density versus pressure at constant temperature
    //! \param  pressure    pressure (p)
    //! \param  temperature temperature (T)
    //! \return derivative : \f$ \left. \frac{\partial \rho}{\partial p} \right)_T = \frac{(\gamma -1) c_v T}{\left((\gamma - 1)* c_v * T + b * (p + p_{\infty})\right)^2} \f$
    double drhodpcT(const double& pressure, const double& temperature) const override;
    ...
    void verifyPressure(const double& pressure, const std::string& message = "") const override;
    ...
    void verifyAndModifyPressure(double& pressure) const override;
    //! \brief   Modify the density if its value is above a limit value equal to \f$ \frac{1.}{b} - 1. \f$ and then correct the volume fraction based on mass
    //! \param   mass		    mass (continuity)
    //! \param   alpha		  volume fraction (\f$ \alpha \f$)
    //! \param   density		density (\f$ \rho \f$)
    void verifyAndCorrectDensityMax(const double& mass, double& alpha, double& density) const override;
    //! \brief   Modify the density if its value is above a limit value equal to \f$ \frac{1.}{b} - 1. \f$
    //! \param   density		density (\f$ \rho \f$)
    ...
    //!return   \f$ \ "NASG" \f$
    TypeEOS getType() const override { return TypeEOS::NASG; };
