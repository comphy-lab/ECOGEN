...
EosSG::EosSG(std::vector<std::string>& nameParameterEos, int& number) : Eos(number)
{
...
double EosSG::computeAcousticImpedance(const double& density, const double& pressure) const { return sqrt(density * m_gamma * (pressure + m_pInf)); }
...
{
  return (initialPressure + m_pInf) * ((m_gamma + 1.) * finalDensity - (m_gamma - 1.) * initialDensity) /
           std::max(((m_gamma + 1.) * initialDensity - (m_gamma - 1.) * finalDensity), epsilonAlphaNull) -
         m_pInf;
}
...
double EosSG::computeEnthalpyIsentropic(const double& initialPressure, const double& initialDensity, const double& finalPressure, double* dhdp) const
{
  ...
  //AF//TODO// Precompute m_gamma/(m_gamma-1)
  double finalEnthalpy(m_gamma * (finalPressure + m_pInf) / (m_gamma - 1.) / std::max(finalRho, epsilonAlphaNull) + m_eRef);
  if (dhdp != NULL)
    *dhdp = m_gamma / (m_gamma - 1.) * (finalRho - (finalPressure + m_pInf) * drho) / std::max((finalRho * finalRho), epsilonAlphaNull);
...
double EosSG::computeDensityEnergySaturation(const double& pressure, const double& rho, const double& drhodp, double* drhoedp) const
{
  double rhoe;
  if (drhoedp != NULL) {
    *drhoedp = 1. / (m_gamma - 1.) + drhodp * m_eRef;
  }
...
{
  //AF//TODO use oneOverGamMinusOne to compute  gamPinfOverGamMinusOne
...
double EosSG::dvdhcp(const double& pressure) const { return (m_gamma - 1.) / m_gamma / std::max((pressure + m_pInf), epsilonAlphaNull); }

//***********************************************************************

double EosSG::drhodpcT(const double& /*pressure*/, const double& temperature) const
{
  return 1. / std::max((temperature * m_cv * (m_gamma - 1.)), epsilonAlphaNull);
}
