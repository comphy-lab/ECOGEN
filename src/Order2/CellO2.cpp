...
CellO2::~CellO2()
{
  if (m_vecPhasesO2) { // when the code fails before model construction, m_vecPhasesO2 may not be allocated while numberPhases is already set
    for (int k = 0; k < numberPhases; k++) {
      if (m_vecPhasesO2[k]) delete m_vecPhasesO2[k];
    }
  }
...
void CellO2::saveCons()
{
  ...
  for (int k = 0; k < numberTransports; k++) {
    m_consTransportsSauvegarde[k].setValue(m_consTransports[k].getValue());
  }
}
...
void CellO2::getBackCons()
{
  ...
  for (int k = 0; k < numberTransports; k++) {
    m_consTransports[k].setValue(m_consTransportsSauvegarde[k].getValue());
  }
}
...
void CellO2::predictionOrdre2(const double& dt, Symmetry* symmetry)
{
  ...
  symmetry->addSymmetricTerms(this);             //m_cons (sum of fluxes) is incremented by the cylindrical or
                                                 // spherical symmetric terms from primitive variables at time n
...
void CellO2::correctionEnergy(Prim type)
{
  switch (type) {
  case vecPhases:
    ...
    break;
  case vecPhasesO2:                                                   //Used during prediction step
    m_mixtureO2->totalEnergyToInternalEnergy(m_vecQuantitiesAddPhys); //Building specific internal energy from totale one
...
void CellO2::fulfillState(Prim type)
{
  ...
  switch (type) {
  case vecPhases:
  case resume: //Identical to cell first order
    ...
    //  - Riemann solver: No need to reconstruct the total energy there because it isn't grabbed during the Riemann problem.
    //    The total energy is directly reconstruct there. The reason is to avoid calculations on the gradients of
    //    additional physics which are not necessary and furthermore wrongly computed.
    ...
    break;
  case vecPhasesO2: //Only usefull for the prediction step after relaxations and the parallel with second order
...
Phase* CellO2::getPhase(const int& phaseNumber, Prim type) const
{
  ...
  case vecPhases:
    ...
    break;
  case vecPhasesO2:
    return m_vecPhasesO2[phaseNumber];
    break;
  default:
    return 0;
    break;
...
Transport& CellO2::getTransport(const int& numTransport, Prim type) const
{
  ...
  case vecPhases:
    ...
    break;
  case vecPhasesO2:
    return m_vecTransportsO2[numTransport];
    break;
  default:
    return m_vecTransports[numTransport];
    break; //FP//TODO// trouver un moyen plus intelligent de faire les renvoi par defaut sur objets.
  }
}
...
void CellO2::setTransport(double value, int& numTransport, Prim type)
{
  ...
  case vecPhases:
    ...
    break;
  case vecPhasesO2:
    m_vecTransportsO2[numTransport].setValue(value);
    break;
  default:
    break;
  }
