...
double Mixture::computeTsat(const Eos* eosLiq, const Eos* eosVap, const double& pressure, double* dTsat)
{
  ...
  if (eosLiq->getType() != TypeEOS::IG && eosLiq->getType() != TypeEOS::SG) {
    Errors::errorMessage("Only IG and SG permitted in thermal equilibrium model: MixPTUEq::computeTsat");
  }
  if (eosVap->getType() != TypeEOS::IG && eosVap->getType() != TypeEOS::SG) {
    Errors::errorMessage("Only IG and SG permitted in thermal equilibrium model: MixPTUEq::computeTsat");
  }
...
{
  if (eosLiq->getType() != TypeEOS::IG && eosLiq->getType() != TypeEOS::SG) {
    Errors::errorMessage("Only IG and SG permitted saturation pressure computation");
  }
  if (eosVap->getType() != TypeEOS::IG && eosVap->getType() != TypeEOS::SG) {
    Errors::errorMessage("Only IG and SG permitted saturation pressure computation");
  }
