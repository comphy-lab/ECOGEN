...
EosIG::EosIG(std::vector<std::string>& nameParameterEos, int& number) : Eos(number)
{
...
double EosIG::computePressure(const double& density, const double& energy) const { return (m_gamma - 1.) * density * (energy - m_eRef); }
...
double EosIG::computeAcousticImpedance(const double& density, const double& pressure) const { return sqrt(density * m_gamma * pressure); }
...
double EosIG::dvdhcp(const double& pressure) const { return (m_gamma - 1.) / m_gamma / std::max(pressure, epsilonAlphaNull); }

//***********************************************************************

double EosIG::drhodpcT(const double& /*pressure*/, const double& temperature) const
{
  return 1. / std::max(((m_gamma - 1.) * m_cv * temperature), epsilonAlphaNull);
}
