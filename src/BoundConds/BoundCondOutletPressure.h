...
class BoundCondOutletPressure : public BoundCond
{
  public:
    BoundCondOutletPressure(
      int numPhysique, tinyxml2::XMLElement* element, const int& numbTransports, std::vector<std::string> nameTransports, std::string fileName);
    BoundCondOutletPressure(const BoundCondOutletPressure& Source, const int& lvl = 0); //Copy ctor (useful for AMR)
    ~BoundCondOutletPressure() override;
    ...
    int whoAmI() const override { return OUTLETPRESSURE; };
