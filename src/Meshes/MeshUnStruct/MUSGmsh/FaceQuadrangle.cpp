...
FaceQuadrangle::FaceQuadrangle(const int& numNode1, const int& numNode2, const int& numNode3, const int& numNode4, int tri) : FaceNS(NUMBERNODES)
{
...
FaceQuadrangle::~FaceQuadrangle() { delete[] m_numNodesOrigine; }
...
void FaceQuadrangle::computeSurface(const Coord* nodes)
{
  ...
  double c(v3.norm());
  double d(v4.norm());
  //AF//TODO//Look if possible to reduce number of call to sqrt
...
{
  Coord v1;
  v1.setFromSubtractedVectors(nodes[m_numNodes[0]], nodes[m_numNodes[1]]);
  Coord v2;
  v2.setFromSubtractedVectors(nodes[m_numNodes[0]], nodes[m_numNodes[2]]);
  ...
  Coord v3;
  v3.setFromSubtractedVectors(nodes[m_numNodes[0]], nodes[numNodeOther]);
