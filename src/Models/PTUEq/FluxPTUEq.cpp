...
FluxPTUEq::FluxPTUEq(const int& numbPhases) { m_mass = new double[numbPhases]; }

//***********************************************************************

FluxPTUEq::~FluxPTUEq() { delete[] m_mass; }

//***********************************************************************

void FluxPTUEq::printFlux() const { std::cout << m_mass << " " << m_momentum.getX() << std::endl; }
...
void FluxPTUEq::setBufferFlux(Cell& cell) { static_cast<FluxPTUEq*>(fluxBuff)->buildCons(cell.getPhases(), cell.getMixture()); }
...
void FluxPTUEq::buildPrim(Phase** phases, Mixture* mixture)
{
  ...
  internalEnergy = m_energMixture / rhoMel -
                   0.5 * (mixture->getU() * mixture->getU() + mixture->getV() * mixture->getV() + mixture->getW() * mixture->getW());
