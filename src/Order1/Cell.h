...
class Cell
{
  public:
    ...
    void buildPrim();
    void buildCons();
    virtual void correctionEnergy(Prim /*type*/ = vecPhases);
    ...
    {
      return m_vecQuantitiesAddPhys[numQPA];
    ...
    virtual void computeLocalSlopes(CellInterface& /*cellInterface*/,
    ...
    virtual void computeLocalSlopesLimite(CellInterface& /*cellInterface*/,
                                          Limiter& /*globalLimiter*/,
                                          Limiter& /*interfaceLimiter*/,
                                          Limiter& /*globalVolumeFractionLimiter*/,
                                          Limiter& /*interfaceVolumeFractionLimiter*/,
    ...
    void chooseRefine(const double& xiSplit,
                      const int& nbCellsY,
                      const int& nbCellsZ,
                      const std::vector<AddPhys*>& addPhys,
                      int& nbCellsTotalAMR);                                    /*!< Choice for refinement of parent cell */
    void chooseUnrefine(const double& xiJoin, int& nbCellsTotalAMR);            /*!< Choice for unrefinement of parent cell */
    void refineCellAndCellInterfaces(const int& nbCellsY,
                                     const int& nbCellsZ,
                                     const std::vector<AddPhys*>& addPhys,
                                     const bool& refineExternalCellInterfaces); /*!< Refine parent cell by creation of children cells */
    ...
    void buildLvlCellsAndLvlInternalCellInterfacesArrays(
      std::vector<Cell*>* cellsLvl,
      std::vector<CellInterface*>* cellInterfacesLvl);                          /*!< Build new arrays of cells and cell interfaces for level (lvl+1),
                                                                                     only internal cell interfaces are added here */
    ...
    void fillBufferVector(
      double* buffer, int& counter, const int& lvl, const int& neighbour, const int& dim, Variable nameVector, int num = 0, int index = -1) const;
    ...
    {
      Errors::errorMessage("fillBufferSlopes not available for Cell");
    }; /*!< Does nothing for first order cells */
    virtual void getBufferSlopes(double* /*buffer*/, int& /*counter*/, const int& /*lvl*/)
    {
      Errors::errorMessage("getBufferSlopes not available for Cell");
    }; /*!< Does nothing for first order cells */
    virtual bool isCellGhost() const { return false; };
    bool hasNeighboringGhostCellOfCPUneighbour(const int& neighbour) const;      /*!< Return a bool that is true if the cell has
                                                                                      a neighboring ghost cell corresponding to CPU "neighbour" */
    int numberOfNeighboringGhostCellsOfCPUneighbour(const int& neighbour) const; /*!< Return the number of neighboring ghost cells
                                                                                      corresponding to CPU "neighbour" this cell has */
    ...
    //--------------------------
    void chooseRefineDeraffineGhost(
      const int& nbCellsY,
      const int& nbCellsZ,
      const std::vector<AddPhys*>& addPhys,
      std::vector<Cell*>* cellsLvlGhost); /*!< Choice for refinement, unrefinement of the ghost parent cell + Update of ghost cell vector for lvl+1 */
    void
    refineCellAndCellInterfacesGhost(const int& nbCellsY,
                                     const int& nbCellsZ,
                                     const std::vector<AddPhys*>& addPhys); /*!< Refinement of parent ghost cell by creation of children ghost cells */
    ...
    void fillNumberElementsToSendToNeighbour(int& numberElementsToSendToNeighbor,
    ...
    void getDataToReceiveAndRefine(std::vector<double>& dataToReceive,
