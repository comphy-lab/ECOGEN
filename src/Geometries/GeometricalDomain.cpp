...
GeometricalDomain::~GeometricalDomain()
{
  for (unsigned int k = 0; k < m_vecPhases.size(); k++) {
    delete m_vecPhases[k];
  }
  m_vecPhases.clear();
  delete m_mixture;
  m_vecTransports.clear();
}
...
void GeometricalDomain::fillIn(Cell* cell) const
{
  ...
  if (belongs) {
    ...
    if (m_physicalEntity == -1) { // Test for immersed boundaries
      cell->setWall(true);
    }
    else {
      cell->setWall(false);
