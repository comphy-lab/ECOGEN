...
ModPTUEq::ModPTUEq(const int& numbTransports, const int& numbPhases) : Model(NAME, numbTransports)
{
...
void ModPTUEq::allocateCons(Flux** cons) { *cons = new FluxPTUEq(numberPhases); }

//***********************************************************************

void ModPTUEq::allocatePhase(Phase** phase) { *phase = new PhasePTUEq; }

//***********************************************************************

void ModPTUEq::allocateMixture(Mixture** mixture) { *mixture = new MixPTUEq; }

//***********************************************************************

void ModPTUEq::allocatePhaseGradient(GradPhase** phase) { *phase = new GradPhasePTUEq; }

//***********************************************************************

void ModPTUEq::allocateMixtureGradient(GradMixture** mixture) { *mixture = new GradMixPTUEq; }
...
void ModPTUEq::solveRiemannIntern(
  Cell& cellLeft, Cell& cellRight, const double& dxLeft, const double& dxRight, double& dtMax, std::vector<double>& boundData) const
{
  ...
  // clang-format off
  double uL = cellLeft.getMixture()->getVelocity().getX(), cL = cellLeft.getMixture()->getMixSoundSpeed(),
         pL = cellLeft.getMixture()->getPressure(), rhoL = cellLeft.getMixture()->getDensity();
  double uR = cellRight.getMixture()->getVelocity().getX(), cR = cellRight.getMixture()->getMixSoundSpeed(),
...
void ModPTUEq::solveRiemannWall(Cell& cellLeft, const double& dxLeft, double& dtMax, std::vector<double>& boundData) const
{
  ...
  double uL = cellLeft.getMixture()->getVelocity().getX(), cL = cellLeft.getMixture()->getMixSoundSpeed(), pL = cellLeft.getMixture()->getPressure(),
...
void ModPTUEq::solveRiemannInletTank(Cell& cellLeft,
                                     const double& dxLeft,
                                     double& dtMax,
                                     const double* ak0,
                                     const double* rhok0,
                                     ...
                                     std::vector<double>& boundData) const
{
  ...
  double uL = cellLeft.getMixture()->getVelocity().getX(), cL = cellLeft.getMixture()->getMixSoundSpeed(), pL = cellLeft.getMixture()->getPressure(),
         rhoL = cellLeft.getMixture()->getDensity(), TL = cellLeft.getMixture()->getTemperature();
  double uyL = cellLeft.getMixture()->getVelocity().getY(), uzL = cellLeft.getMixture()->getVelocity().getZ(),
         EL = cellLeft.getMixture()->getEnergy() + 0.5 * cellLeft.getMixture()->getVelocity().squaredNorm();
...
const double& ModPTUEq::getSM() { return static_cast<FluxPTUEq*>(fluxBuff)->m_sM; }
