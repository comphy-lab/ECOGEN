...
GOLine::GOLine(const Coord& vertex, const Coord& vecDir) : GeometricObject(LINE, vertex), m_vecDir(vecDir)
{
  if (vecDir.norm() < 1e-6) {
...
{
  Coord vec;
  vec.setFromSubtractedVectors(m_point, vertex);
  if (std::fabs(vec.scalar(m_vecDir)) < 1e-6) {
    return 0.;
  }
  else {
    return (vec.cross(m_vecDir)).norm();
  }
}
