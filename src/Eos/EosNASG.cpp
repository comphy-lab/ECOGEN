...
EosNASG::EosNASG(std::vector<std::string>& nameParameterEos, int& number) : Eos(number)
{
...
{
  return (pressure + m_pInf) * std::max(1. - density * m_b, epsilonAlphaNull) / (m_gamma - 1.) / m_cv / std::max(density, epsilonAlphaNull);
}
...
{
  return (pressure + m_gamma * m_pInf) * std::max(1. - density * m_b, epsilonAlphaNull) / (m_gamma - 1.) / std::max(density, epsilonAlphaNull) +
         m_eRef;
}
...
double EosNASG::computeInterfaceSoundSpeed(const double& density, const double& interfacePressure, const double& pressure) const
{
  return sqrt(((m_gamma - 1.) * interfacePressure + pressure + m_gamma * m_pInf) / std::max(density * (1. - density * m_b), epsilonAlphaNull));
}

//***********************************************************************

double EosNASG::computeAcousticImpedance(const double& density, const double& pressure) const
{
  return sqrt(density * m_gamma * (pressure + m_pInf) / std::max(1. - density * m_b, epsilonAlphaNull));
}

//***********************************************************************

double EosNASG::computeDensityTimesInterfaceSoundSpeedSquare(const double& density, const double& interfacePressure, const double& pressure) const
{
  return ((m_gamma - 1.) * interfacePressure + pressure + m_gamma * m_pInf) / std::max(1. - density * m_b, epsilonAlphaNull);
}
...
{
  return (initialPressure + m_pInf) * std::pow(finalDensity / std::max(initialDensity, epsilonAlphaNull), m_gamma) *
           std::pow((1. - initialDensity * m_b) / (1. - finalDensity * m_b), m_gamma) -
         m_pInf;
}
...
{
  return (initialDensity * (2. * m_gamma * m_pInf + initialPressure * (m_gamma - 1.)) -
          finalDensity * (2. * m_gamma * m_pInf + initialPressure * (m_gamma + 1.)) + 2. * m_b * initialDensity * initialPressure) /
         TB->returnNonZeroValue(finalDensity * (m_gamma - 1.) - initialDensity * (m_gamma + 1.) + 2. * m_b * initialDensity);
}

//***********************************************************************

double
EosNASG::computeDensityIsentropic(const double& initialPressure, const double& initialDensity, const double& finalPressure, double* drhodp) const
{
  //AF//TODO// Precompute 1/m_gamma
  double finalDensity(initialDensity * std::pow(finalPressure + m_pInf, 1. / m_gamma) /
                      TB->returnNonZeroValue(std::pow(initialPressure + m_pInf, 1. / m_gamma) * (1. - initialDensity * m_b) +
                                             initialDensity * m_b * std::pow(finalPressure + m_pInf, 1. / m_gamma)));
...
double EosNASG::computeDensityHugoniot(const double& initialPressure, const double& initialDensity, const double& finalPressure, double* drhodp) const
{
  ...
  double denom((m_gamma - 1.) * (finalPressure + m_pInf) + (m_gamma + 1.) * (initialPressure + m_pInf) +
               2. * m_b * initialDensity * (finalPressure - initialPressure));
  double finalDensity(initialDensity * num / TB->returnNonZeroValue(denom));
  if (drhodp != NULL)
    *drhodp = initialDensity * 4. * (m_gamma) * (initialPressure + m_pInf) * (1. - m_b * initialDensity) / TB->returnNonZeroValue(denom * denom);
  return finalDensity;
}
...
double EosNASG::computeDensityPfinal(const double& initialPressure, const double& initialDensity, const double& finalPressure, double* drhodp) const
{
  ...
  double finalDensity(initialDensity * num / TB->returnNonZeroValue(denom));
  if (drhodp != NULL)
    *drhodp = initialDensity * m_gamma * (initialPressure + m_pInf - m_b * initialDensity * (initialPressure + m_pInf)) /
              TB->returnNonZeroValue(denom * denom);
  return finalDensity;
}
...
double EosNASG::computeEnthalpyIsentropic(const double& initialPressure, const double& initialDensity, const double& finalPressure, double* dhdp) const
{
  ...
  double finalEnthalpy(m_gamma * (finalPressure + m_pInf) * (1. - finalRho * m_b) / (m_gamma - 1.) / std::max(finalRho, epsilonAlphaNull) +
                       m_b * finalPressure + m_eRef);
  if (dhdp != NULL)
    *dhdp = m_gamma / (m_gamma - 1.) * ((finalRho - (finalPressure + m_pInf) * drho) / std::max((finalRho * finalRho), epsilonAlphaNull) - m_b) + m_b;
...
double EosNASG::computeDensitySaturation(const double& pressure, const double& Tsat, const double& dTsatdP, double* drhodp) const
{
  ...
  if (drhodp != NULL) {
    ...
    *drhodp /= std::max((((m_gamma - 1.) * m_cv * Tsat + m_b * (pressure + m_pInf)) * ((m_gamma - 1.) * m_cv * Tsat + m_b * (pressure + m_pInf))),
                        epsilonAlphaNull);
  }
...
double EosNASG::computeDensityEnergySaturation(const double& pressure, const double& rho, const double& drhodp, double* drhoedp) const
{
  double rhoe;
  if (drhoedp != NULL) {
    *drhoedp = (1. - rho * m_b) / (m_gamma - 1.) + drhodp * (m_eRef - m_b * (pressure + m_gamma * m_pInf) / (m_gamma - 1.));
  }
...
{
  return (-(m_gamma - 1.) / m_gamma / std::max(((pressure + m_pInf) * (pressure + m_pInf)), epsilonAlphaNull) *
          (m_b * (pressure + m_pInf) + enthalpy - m_b * pressure - m_eRef));
}

//***********************************************************************

double EosNASG::dvdhcp(const double& pressure) const { return (m_gamma - 1.) / m_gamma / std::max((pressure + m_pInf), epsilonAlphaNull); }

//***********************************************************************

double EosNASG::drhodpcT(const double& pressure, const double& temperature) const
{
  double buff((m_gamma - 1.) * m_cv * temperature + m_b * (pressure + m_pInf));
  return (m_gamma - 1.) * m_cv * temperature / std::max(buff * buff, epsilonAlphaNull);
}
...
void EosNASG::verifyAndCorrectDensityMax(const double& mass, double& alpha, double& density) const
{
  if (density > 1. / m_b - 1.) {
    density = 1. / m_b - 1.;
    alpha   = mass / density;
