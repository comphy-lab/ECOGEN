...
APUEqSurfaceTension::APUEqSurfaceTension(
  XMLElement* element, int& numberQPA, std::vector<std::string> nameTransports, std::vector<std::string> namePhases, std::string nameFichier)
{
  ...
  for (t = 0; t < nameTransports.size(); t++) {
    if (m_nameTransportAssociated == nameTransports[t]) {
      break;
    }
  }
  if (t != nameTransports.size()) {
    m_numTransportAssociated = t;
  }
  else {
    Errors::errorMessage("Transport equation not found for surface tension");
  }
  ...
  if (sousElement2 != NULL) {
    ...
    for (p = 0; p < namePhases.size(); p++) {
      if (m_namePhaseAssociated == namePhases[p]) {
        break;
      }
    }
    if (p != namePhases.size()) {
      m_numPhaseAssociated = p;
    }
    else {
      Errors::errorMessage("Equation of phase not found for surface tension (reinitialization)");
    }
  }
...
void APUEqSurfaceTension::addQuantityAddPhys(Cell* cell) { cell->getVecQuantitiesAddPhys().push_back(new QAPSurfaceTension(this)); }
...
void APUEqSurfaceTension::solveFluxAddPhysBoundary(CellInterface* cellInterface)
{
  ...
  case NONREFLECTING:
    this->solveFluxSurfaceTensionNonReflecting(m_velocityLeft, m_gradCLeft);
    break;
  case WALL:
  case SYMMETRY:
    this->solveFluxSurfaceTensionWall(m_gradCLeft);
    break;
  case OUTLETPRESSURE:
    this->solveFluxSurfaceTensionOutletPressure();
    break;
  case INLETINJSTAGSTATE:
    this->solveFluxSurfaceTensionInletInjStagState();
    break;
  default:
    this->solveFluxSurfaceTensionOther();
  }
...
void APUEqSurfaceTension::addSymmetricTermsRadialAxisOnX(Cell* cell)
{
  ...
  if (normW > 1.e-6) {
    double msigma_on_Wr = m_sigma / (normW * r);
    static_cast<FluxUEq*>(fluxBuff)->m_momentum.setX(-msigma_on_Wr * w1 * w1);
    static_cast<FluxUEq*>(fluxBuff)->m_momentum.setY(-msigma_on_Wr * w1 * w2);
    static_cast<FluxUEq*>(fluxBuff)->m_energMixture = -msigma_on_Wr * (w1 * w1 * cell->getMixture()->getU() + w1 * w2 * cell->getMixture()->getV());
  }
  else {
...
void APUEqSurfaceTension::addSymmetricTermsRadialAxisOnY(Cell* cell)
{
  ...
  if (normW > 1.e-6) {
    //AF//TODO//Precompute sigma_on_wr
    ...
    static_cast<FluxUEq*>(fluxBuff)->m_energMixture = -m_sigma * (w1 * w2 * cell->getMixture()->getU() + w2 * w2 * cell->getMixture()->getV()) /
                                                      normW / r;
  }
  else {
    static_cast<FluxUEq*>(fluxBuff)->m_momentum     = 0.;
...
void APUEqSurfaceTension::reinitializeColorFunction(std::vector<Cell*>* cellsLvl, const int& lvl)
{
  for (unsigned int i = 0; i < cellsLvl[lvl].size(); i++) {
    if (!cellsLvl[lvl][i]->getSplit()) {
      cellsLvl[lvl][i]->reinitializeColorFunction(m_numTransportAssociated, m_numPhaseAssociated);
    }
  }
}
