...
OutputProbeGNU::OutputProbeGNU(std::string casTest, std::string run, XMLElement* element, std::string fileName, Input* entree) : OutputGNU(element)
{
  try {
    //Attributes settings
    m_writeBinary         = false;
...
void OutputProbeGNU::probeDisplacement(const double& dt)
{
  // Find the current cell containing the probe
  if (!m_cell->getSplit()) { //if cell is not split
    m_cellAMR = m_cell;
  }
  else { //if cell is split, locate subCells in sub AMR mesh
    m_cellAMR = locateProbeInAMRSubMesh(m_cell->getChildVector(), m_cell->getChildVector()->size());
  }

  // Displace the lagrangian probe
  m_objet->setPositionPoint(m_objet->getPoint().getX() + m_cellAMR->getMixture()->getU() * dt,
                            m_objet->getPoint().getY() + m_cellAMR->getMixture()->getV() * dt,
                            m_objet->getPoint().getZ() + m_cellAMR->getMixture()->getW() * dt);
}
...
void OutputProbeGNU::locateProbeInMesh(const TypeMeshContainer<Cell*>& cells, const int& nbCells, bool localSeeking)
{
  ...
  if (!localSeeking) {
    ...
    if (Ncpu != 1) {
      ...
      if (std::fabs(minimumAllCPU - minimumDistance) > 1.e-10) {
        m_possessesProbe[rankCpu] = false;
      }
      else {
