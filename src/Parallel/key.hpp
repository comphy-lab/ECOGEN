...
template <int Dim> struct Key
{
  public: // member types
    ...
    template <typename U> using vector_type = math::vector<U, Dim>;
    using coordinate_type                   = vector_type<scalar_coordinate_type>;
    using real_coordinate_type              = vector_type<float_type>;
    struct hash_functor
    {
        std::size_t operator()(Key const& n) const noexcept { return std::hash<value_type>()(n.index_); }
    };
  ...
  public: // Ctors
    Key() noexcept : index_(0) {}

    Key(value_type idx) noexcept : index_(idx) {}

    Key(int x, int y, int z) noexcept : Key(coordinate_type({x, y, z})) {}

    Key(coordinate_type x) noexcept : index_(compute_index(x)) {}
  ...
  public: //Access
    ...
    Key child(int i) const noexcept { return (index_ << Dim) | static_cast<value_type>(i); }
    ...
    {
      os << std::bitset<64>(_k.index_) << " = " << _k.index_ << " coord =( " << _k.coordinate() << " )";
      return os;
    }

  private: //Static
    template <int nDim = Dim> static value_type split_bits(value_type w)
    {
      using tag = std::integral_constant<int, 3>*;
      return split_bits_impl(w, tag(0));
    }
    template <int nDim = Dim> static value_type compress_bits(value_type w)
    {
      using tag = std::integral_constant<int, 3>*;
...
// binary operators
template <int Dim> Key<Dim> operator+(int n, Key<Dim> k) noexcept { return k += n; }

template <int Dim> Key<Dim> operator-(int n, Key<Dim> k) noexcept { return k -= n; }

// relational operators
template <int Dim> constexpr bool operator==(const Key<Dim>& l, const Key<Dim>& r) noexcept { return l.index() == r.index(); }
