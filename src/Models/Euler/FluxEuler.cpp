...
FluxEuler::FluxEuler() : m_mass(0.), m_momentum(0.), m_energ(0.) {}
...
void FluxEuler::printFlux() const { std::cout << m_mass << " " << m_momentum.getX() << " " << m_energ << std::endl; }
...
void FluxEuler::setBufferFlux(Cell& cell) { static_cast<FluxEuler*>(fluxBuff)->buildCons(cell.getPhases(), cell.getMixture()); }
...
void FluxEuler::prepSourceTermsPlaneWave(const Cell* cell, const double& sourceMomentum, const Coord& pulseDirection)
{
  Phase* phase(cell->getPhase(0));
  m_mass = sourceMomentum / phase->getSoundSpeed();
  ...
  m_energ = sourceMomentum * phase->getSoundSpeed() / (phase->getEos()->getGamma() - 1.);
}
