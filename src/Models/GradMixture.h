...
class GradMixture
{
  public:
    ...
    {
      Errors::errorMessage("computeDistanceGradientScalarProduct not available for required GradMixture");
    };
    virtual void limitGradients(const Mixture& /*gradientLimiter*/)
    {
      Errors::errorMessage("limitGradients not available for required GradMixture");
    };

    // -- O2 parallel --
    virtual int numberOfTransmittedGradients() const
    {
      Errors::errorMessage("numberOfTransmittedGradients not available for required GradMixture");
