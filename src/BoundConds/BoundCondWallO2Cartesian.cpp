...
BoundCondWallO2Cartesian::BoundCondWallO2Cartesian(const BoundCondWallO2Cartesian& Source, const int& lvl) :
  BoundCondWall(Source, lvl), m_vecPhasesSlopes(0), m_mixtureSlopes(0), m_vecTransportsSlopes(0)
{}

//****************************************************************************

BoundCondWallO2Cartesian::BoundCondWallO2Cartesian(int numPhysique, tinyxml2::XMLElement* element, std::string fileName) :
  BoundCondWall(numPhysique, element, fileName), m_vecPhasesSlopes(0), m_mixtureSlopes(0), m_vecTransportsSlopes(0)
{}

//****************************************************************************

BoundCondWallO2Cartesian::BoundCondWallO2Cartesian(int numPhysique) :
  BoundCondWall(numPhysique), m_vecPhasesSlopes(0), m_mixtureSlopes(0), m_vecTransportsSlopes(0)
{}
...
{
  if (m_vecPhasesSlopes) { // Needed for cases where the vector is not allocated (dealloc of m_limXm for example)
...
void BoundCondWallO2Cartesian::allocateSlopes(int& /*allocateSlopeLocal*/)
{
  ...
  // We set phases from the left cell (because the right cell doesn't exist for the limits)
  // Necessary because we need to know the phase type (for example: PhasePUEq, etc.)
  // Then all slopes are set to zero
  for (int k = 0; k < numberPhases; k++) {
                                            ...
                                            Prim type)
{
  ...
  if (m_face->getNormal().getX() < 0. || m_face->getNormal().getY() < 0. || m_face->getNormal().getZ() < 0.) {
    distanceGauche = -distanceGauche;
  }

  //Left side of interface extrapolation
  double epsInterface(1.e-4);
  m_cellLeft->computeLocalSlopesLimite(
    *this, globalLimiter, interfaceLimiter, globalVolumeFractionLimiter, interfaceVolumeFractionLimiter, epsInterface);
  for (int k = 0; k < numberPhases; k++) {
...
Phase* BoundCondWallO2Cartesian::getSlopesPhase(const int& phaseNumber) const { return m_vecPhasesSlopes[phaseNumber]; }

//***********************************************************************

Mixture* BoundCondWallO2Cartesian::getSlopesMixture() const { return m_mixtureSlopes; }

//***********************************************************************

Transport* BoundCondWallO2Cartesian::getSlopesTransport(const int& numberTransport) const { return &m_vecTransportsSlopes[numberTransport]; }

//***************************************************************************
//******************************AMR Method***********************************
//***************************************************************************

void BoundCondWallO2Cartesian::creerCellInterfaceChild() { m_cellInterfacesChildren.push_back(new BoundCondWallO2Cartesian(*this, m_lvl + 1)); }
