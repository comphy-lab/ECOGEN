...
double Tensor::trace() const { return m_array[XX] + m_array[YY] + m_array[ZZ]; }
...
double Tensor::infinityNorm() const
{
  double norm    = 0.;
  double row_sum = 0.;
  norm           = std::fabs(m_array[XX]) + std::fabs(m_array[XY]) + std::fabs(m_array[XZ]);
  row_sum        = std::fabs(m_array[YX]) + std::fabs(m_array[YY]) + std::fabs(m_array[YZ]);
  norm           = std::max(norm, row_sum);
  row_sum        = std::fabs(m_array[ZX]) + std::fabs(m_array[ZY]) + std::fabs(m_array[ZZ]);
  norm           = std::max(norm, row_sum);
  return norm;
}
...
void Tensor::exponentialPade(Tensor& tensorExpPade)
{
  double normA(this->infinityNorm());
  int s   = std::max(0, static_cast<int>(ceil(log2(normA / 0.5))));
  (*this) = 1. / pow(2., s) * (*this);
  this->matrixProduct((*this), tensorX2);
  tensorX2.matrixProduct(tensorX2, tensorX4);
  tensorX4.matrixProduct(tensorX4, tensorX6);
  tensorBuff = 30. * tensorX6 + 2520. * tensorX4;
  this->matrixProduct(tensorBuff, tensorBuff2);

  tensorU = tensorBuff2 + 281160. * (*this);
  tensorV = 30. * tensorX6 + 2520. * tensorX4 + 420. * tensorX2 + tensorIdentity + 277200. * tensorX6;
  tensorP = tensorU + tensorV;
  tensorQ = tensorV - tensorU;
  tensorQ.inverse(tensorBuff2);
  tensorBuff2.matrixProduct(tensorP, tensorExpPade);

  for (int i = 0; i < s; ++i) {
    tensorExpPade.matrixProduct(tensorExpPade, tensorBuff);
    tensorExpPade = tensorBuff;
  }
}
  ...
  while (b2 > abserr) {
    for (int i = 0; i < 2; i++) {
      for (int j = i + 1; j < 3; j++) {
        ...
        if (std::fabs(eigenvalues.m_array[ji]) > 1.e-25) {
