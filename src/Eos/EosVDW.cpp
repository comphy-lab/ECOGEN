...
EosVDW::EosVDW(std::vector<std::string>& nameParameterEos, int& number) : Eos(number)
{
...
{
  return (m_p0 + m_a * m_rho0 * m_rho0) * std::pow((1. / m_rho0 - m_b) / (1. / density - m_b), m_gamma) - m_a * density * density;
}
...
{
  //AF//TODO// Precompute 1./density
  double dpdrho = (this->computePressure(density, temperature) + m_a * density * density) / (1. / density - m_b) * m_gamma / (density * density) -
                  2. * m_a * density;
