...
{
  this->meshStretching();
  this->initializeGeometrieAMR(cells, cellsGhost, cellInterfaces, resumeSimulation, ordreCalcul);
                                              ...
                                              std::string ordreCalcul)
{
  ...
  if (resumeSimulation == 0) {
    m_decomp = decomposition::Decomposition(physicalDomainSizes);
  }
  else {
    m_decomp.updatePhysicalDomainSizes(physicalDomainSizes);
  }
  auto keys = m_decomp.initialize(Ncpu, rankCpu, resumeSimulation);
                                                              ...
                                                              std::string ordreCalcul)
{
  ...
  for (unsigned int i = 0; i < cells.size(); ++i) {
    ...
    for (int idx = 0; idx < 2 * m_problemDimension; idx++) {
      ...
      normal.setXYZ(static_cast<double>(offset[0]), static_cast<double>(offset[1]), static_cast<double>(offset[2]));
      ...
      else //Offset is an internal cell (ghost or not)
      {
        ...
        if (offset[0] > 0 || offset[1] > 0 || offset[2] > 0) //Positive offset
        {
          ...
          else //Neighbor cell is a ghost cell
          {
            ...
            auto it2 = std::find_if(cellsGhost.begin(), cellsGhost.end(), [&nKey](Cell* _k0) { return _k0->getElement()->getKey() == nKey; });

            if (it2 == cellsGhost.end()) //Ghost cell does not exist
            {
              //Create ghost cell and update cell interface
              if (ordreCalcul == "FIRSTORDER") {
                cellsGhost.push_back(new CellGhost);
              }
              else {
                cellsGhost.push_back(new CellO2GhostCartesian);
              }
              m_elements.push_back(new ElementCartesian());
              ...
              auto it3 = std::find_if(parallel.getElementsToSend(neighbour).begin(), parallel.getElementsToSend(neighbour).end(), [&cKey](Cell* _k0) {
              ...
              double lCFL(1.e10);
              if (m_numberCellsX != 1) {
                lCFL = std::min(lCFL, m_dXi[nix]);
              }
              if (m_numberCellsY != 1) {
                lCFL = std::min(lCFL, m_dYj[niy]);
              }
              if (m_numberCellsZ != 1) {
                lCFL = std::min(lCFL, m_dZk[niz]);
              }
              if (m_problemDimension > 1) lCFL *= 0.6;
    ...
    for (int lvl = 0; lvl <= m_lvlMax; lvl++) {
      ...
      for (unsigned int i = 0; i < cellsLvl[lvl].size(); i++) {
        if (!cellsLvl[lvl][i]->getSplit()) {
          cellsLvl[lvl][i]->completeFulfillState();
        }
      }
    }
    if (Ncpu > 1) {
      this->parallelLoadBalancingAMR(cellsLvl, cellsLvlGhost, cellInterfacesLvl, ordreCalcul, addPhys, eos, nbCellsTotalAMR, solidDomains, true);
    }
  }
}
                                            ...
                                            Eos** eos)
{
  ...
  for (unsigned int i = 0; i < cellInterfacesLvl[lvl].size(); i++) {
    cellInterfacesLvl[lvl][i]->computeXi(m_criteriaVar, m_varRho, m_varP, m_varU, m_varAlpha);
  }
  // bool varP2 = m_varP;
  ...
  for (int iterDiff = 0; iterDiff < 2; iterDiff++) { //Arbitrary number of iterations
    ...
    //Mise a zero cons xi
    for (unsigned int i = 0; i < cellsLvl[lvl].size(); i++) {
      cellsLvl[lvl][i]->setToZeroConsXi();
    }
    ...
    for (unsigned int i = 0; i < cellsLvl[lvl].size(); i++) {
      cellsLvl[lvl][i]->timeEvolutionXi();
    }
    if (Ncpu > 1) {
      parallel.communicationsXi(lvl);
    }
  ...
  if (lvl < m_lvlMax) {
    ...
    //-------------------------------------------
    for (unsigned int i = 0; i < cellsLvl[lvl].size(); i++) {
      cellsLvl[lvl][i]->chooseRefine(m_xiSplit, m_numberCellsY, m_numberCellsZ, addPhys, nbCellsTotalAMR);
    }

    //4) Deraffinement des cells et cell interfaces
    //---------------------------------------------
    for (unsigned int i = 0; i < cellsLvl[lvl].size(); i++) {
      cellsLvl[lvl][i]->chooseUnrefine(m_xiJoin, nbCellsTotalAMR);
    }
    ...
    if (Ncpu > 1) {
      ...
      for (unsigned int i = 0; i < cellsLvlGhost[lvl].size(); i++) {
        cellsLvlGhost[lvl][i]->chooseRefineDeraffineGhost(m_numberCellsY, m_numberCellsZ, addPhys, cellsLvlGhost);
      }
      ...
      //----------------------------------------------------------------
      parallel.communicationsNumberGhostCells(
        lvlPlus1); //Communication des numbers d'elements a envoyer et a recevoir de chaque cote de la limite parallele
...
std::string MeshCartesianAMR::whoAmI() const { return "CARTESIAN_AMR"; }
    ...
    for (unsigned int i = 0; i < cellsLvl[lvl].size(); i++) {
      if (!cellsLvl[lvl][i]->getSplit()) {
        if (var > 0) { //We want to get the scalar data
          if (phase >= 0) {
            //data de phases
            dataset.push_back(cellsLvl[lvl][i]->getPhase(phase)->returnScalar(var));
          }
          else if (phase == -1) {
            //data de mixture
            dataset.push_back(cellsLvl[lvl][i]->getMixture()->returnScalar(var));
          }
          else if (phase == -2) {
            ...
            dataset.push_back(cellsLvl[lvl][i]->getXi());
          }
          else if (phase == -4) {
            dataset.push_back(cellsLvl[lvl][i]->getDensityGradient());
          }
          else if (phase == -5) {
            dataset.push_back(static_cast<double>(rankCpu));
          }
          else if (phase == -6) {
            dataset.push_back(static_cast<double>(cellsLvl[lvl][i]->getElement()->getKey().getIndex()));
          }
          else if (phase == -7) {
            // Saturation pressure
            ...
            Errors::errorMessage("MeshCartesianAMR::getData: unknown number of phase: ", phase);
          }
        }
    ...
    for (unsigned int i = 0; i < cellsLvl[lvl].size(); i++) {
      if (!cellsLvl[lvl][i]->getSplit()) {
        if (var > 0) { //Scalars data are first set
          if (phase >= 0) {
            //phases data
            cellsLvl[lvl][i]->getPhase(phase)->setScalar(var, dataset[iterDataSet++]);
          }
          else if (phase == -1) {
            //mixture data
            cellsLvl[lvl][i]->getMixture()->setScalar(var, dataset[iterDataSet++]);
          }
          else if (phase == -2) {
            //transport data
            cellsLvl[lvl][i]->getTransport(var - 1).setValue(dataset[iterDataSet++]);
          }
          else if (phase == -3) {
            //xi indicator
            cellsLvl[lvl][i]->setXi(dataset[iterDataSet++]);
          }
          else {
            Errors::errorMessage("MeshCartesianAMR::setDataSet: unknown phase number: ", phase);
          }
        }
        else { //We want to get the vector data
...
void MeshCartesianAMR::printDomainDecomposition(std::ofstream& fileStream) { m_decomp.printDomainDecomposition(fileStream); }
...
void MeshCartesianAMR::initializePersistentCommunications(const TypeMeshContainer<Cell*>& cells, std::string ordreCalcul)
{
  ...
  if (ordreCalcul == "SECONDORDER") {
    ...
    //m_numberSlopeVariables -> 2 = +1 for interface detection + 1 for slope index
    m_numberSlopeVariables = numberSlopesPhaseToSend + numberSlopesMixtureToSend + numberTransports + 2;
  }
...
void MeshCartesianAMR::finalizeParallele(const int& lvlMax) { parallel.finalizeAMR(lvlMax); }
                                                ...
                                                bool init)
{
  ...
  do {
    ...
    this->computePotentialBalancing(
      cellsLvl, init, lvl, balance, indicesSendStartGlobal, indicesSendEndGlobal, indicesReceiveStartGlobal, indicesReceiveEndGlobal);
    //} //For levelwise balancing

    if (balance) {
      this->balance(cellsLvl,
                    cellsLvlGhost,
                    cellInterfacesLvl,
                    ordreCalcul,
                    addPhys,
                    eos,
                    nbCellsTotalAMR,
                    indicesSendStartGlobal,
                    indicesSendEndGlobal,
                    indicesReceiveStartGlobal,
  ...
  for (unsigned int i = 0; i < cellsLvl[m_lvlMax].size(); i++) {
    if (!cellsLvl[m_lvlMax][i]->getSplit()) {
      cellsLvl[m_lvlMax][i]->prepareAddPhys();
    }
  }
  for (unsigned int pa = 0; pa < addPhys.size(); pa++) {
    addPhys[pa]->communicationsAddPhys(m_problemDimension, m_lvlMax);
  }
}
                                                 ...
                                                 std::vector<typename decomposition::Key<3>::value_type>& indicesReceiveEndGlobal)
{
  ...
  relativePossibleLoadShiftLocal = std::max(std::max(numberOfCellsToSendStart, numberOfCellsToReceiveStart),
                                            std::max(numberOfCellsToSendEnd, numberOfCellsToReceiveEnd));
  if (localLoad > 1.e-8) {
  ...
  if (init) {
    if (relativePossibleLoadShiftMax > 1.e-10) {
      balance = true;
    }
  }
  else {
    if (relativePossibleLoadShiftMax > 0.05) {
      balance = true;
    }
  }
  ...
  if (numberOfCellsToSendStart > 0) {
    for (unsigned int i = 0; i < cellsLvl[0].size(); ++i) {
      ...
      if (indicesSendStart.size() == (unsigned int)numberOfCellsToSendStart) {
        break;
      }
      //} //For levelwise balancing
    }
  ...
  if (numberOfCellsToSendEnd > 0) {
    for (int i = (int)cellsLvl[0].size() - 1; i >= 0; --i) {
      ...
      if (indicesSendEnd.size() == (unsigned int)numberOfCellsToSendEnd) {
        break;
      }
      //} //For levelwise balancing
    }
                               ...
                               std::vector<GeometricalDomain*>& solidDomains)
{
  ...
  for (unsigned int i = 0; i < numberOfCellsToReceiveStartGlobal; ++i) {
    if (ordreCalcul == "FIRSTORDER") {
      bufferReceiveCellsStart[i] = new Cell;
    }
    else {
      bufferReceiveCellsStart[i] = new CellO2Cartesian;
    }
    m_elements.push_back(new ElementCartesian());
  ...
  for (unsigned int i = 0; i < numberOfCellsToReceiveEndGlobal; ++i) {
    if (ordreCalcul == "FIRSTORDER") {
      bufferReceiveCellsEnd[i] = new Cell;
    }
    else {
      bufferReceiveCellsEnd[i] = new CellO2Cartesian;
    }
    m_elements.push_back(new ElementCartesian());
  ...
  for (unsigned int i = 1; i < cellsLvl[0].size(); i++) {
    if (!m_decomp.areConsecutive(cellsLvl[0][i - 1]->getElement()->getKey().getIndex(), cellsLvl[0][i]->getElement()->getKey().getIndex())) {
  ...
  for (unsigned int b = 0; b < cellInterfacesLvl[0].size(); b++) {
    delete cellInterfacesLvl[0][b];
  }
  for (unsigned int i = 0; i < cellsLvl[0].size(); i++) {
    cellsLvl[0][i]->clearExternalCellInterfaces(m_numberCellsY, m_numberCellsZ);
  }
  for (unsigned int i = 0; i < cellsLvlGhost[0].size(); i++) {
    delete cellsLvlGhost[0][i];
  }
  ...
  //-------------------------------------------------------------------
  for (unsigned int i = 0; i < bufferReceiveCells.size(); i++) {
    bufferReceiveCells[i]->allocate(addPhys);
  }
  for (unsigned int i = 0; i < cellsLvlGhost[0].size(); i++) {
    cellsLvlGhost[0][i]->allocate(addPhys);
  }
  ...
  //--------------------------
  for (unsigned int i = 0; i < cellsLvl[0].size(); i++) {
    cellsLvl[0][i]->fill(solidDomains, m_lvlMax);
  }
  for (unsigned int i = 0; i < cellsLvlGhost[0].size(); i++) {
    cellsLvlGhost[0][i]->fill(solidDomains, m_lvlMax);
  }
  ...
  if (numberOfCellsToReceiveEndGlobal > 0) {
    ...
    for (int lvl = 0; lvl <= m_lvlMax; lvl++) {
      for (unsigned int i = 0; i < bufferReceiveCellsEnd.size(); i++) {
        bufferReceiveCellsEnd[i]->getDataToReceiveAndRefine(
          dataToReceiveEnd, dataSplitToReceiveEnd, lvl, eos, counter, counterSplit, m_numberCellsY, m_numberCellsZ, addPhys);
      }
    }
  }
  if (numberOfCellsToSendEndGlobal > 0) {
  ...
  if (numberOfCellsToReceiveStartGlobal > 0) {
    ...
    for (int lvl = 0; lvl <= m_lvlMax; lvl++) {
      for (unsigned int i = 0; i < bufferReceiveCellsStart.size(); i++) {
        bufferReceiveCellsStart[i]->getDataToReceiveAndRefine(
          dataToReceiveStart, dataSplitToReceiveStart, lvl, eos, counter, counterSplit, m_numberCellsY, m_numberCellsZ, addPhys);
      }
    }
  }
  ...
  //Delete sent cells
  for (unsigned int i = 0; i < bufferSendCells.size(); i++) {
    delete bufferSendCells[i];
  }
      ...
      if (cellsLvl[lvl][i]->getSplit()) {
        for (unsigned int b = 0; b < (unsigned int)cellsLvl[lvl][i]->getCellInterfacesSize(); b++) {
          if (!cellsLvl[lvl][i]->getCellInterface((int)b)->getSplit()) {
            cellsLvl[lvl][i]->getCellInterface((int)b)->raffineCellInterfaceExterne(m_numberCellsY,
                                                                                    m_numberCellsZ,
                                                                                    cellsLvl[lvl][i]->getElement()->getSizeX(),
                                                                                    cellsLvl[lvl][i]->getElement()->getSizeY(),
                                                                                    cellsLvl[lvl][i]->getElement()->getSizeZ(),
                                                                                    cellsLvl[lvl][i],
                                                                                    dim);
          }
        }
