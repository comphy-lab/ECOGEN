...
class Run
{
  ...
  private:
    ...
    TypeMeshContainer<CellInterface*>* m_cellInterfacesLvl; //!<Array of vectors (one per level) of interface objects between cells
                                                            //!< (or between a cell and a physical domain boundary)
    ...
    Limiter* m_globalVolumeFractionLimiter;    //!<Slope limiter type object for second order in space specific to interface advected variables
                                               //!< (alpha, transports)
    Limiter* m_interfaceVolumeFractionLimiter; //!<Slope limiter type object for second order in space specific to interface advected variables
                                               //!< (alpha, transports) and to interface location
    ...
    int m_resumeSimulation;                    //!<File number for resuming a simulation
    int m_resumeAMRsaveFreq;                   //!<Frequency at which a save to resume a simulation is done (usefull only for AMR)
    ...
    bool m_recordPsat;                         //!<Add recording of saturation pressure
                                               //!< (valid for a liquid/vapor couple and predominant phase should be first)
