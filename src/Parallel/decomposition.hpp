...
class Decomposition
{
  ...
  public: //Ctors
    ...
    Decomposition(std::array<int, Dim> _nCells) : nCells_global_(_nCells) {}
    ...
    void updatePhysicalDomainSizes(std::array<int, Dim> _nCells) noexcept { nCells_global_ = _nCells; }

    std::vector<key_type> initialize(int nProcs, int _rank, int resumeSimulation) noexcept
    {
      std::vector<key_type> keys;

      if (resumeSimulation == 0) {
        auto nCells_t = std::accumulate(nCells_global_.begin(), nCells_global_.end(), 1, std::multiplies<int>());
    ...
    {
      return (_key.coordinate()[0] < nCells_global_[0] && _key.coordinate()[1] < nCells_global_[1] && _key.coordinate()[2] < nCells_global_[2]);
    }

    template <class Coord> bool is_inside(const Coord& _coord)
    {
      for (int d = 0; d < Dim; ++d) {
        if (_coord[d] < 0 || _coord[d] >= nCells_global_[d]) return false;
      }
      return true;
    }
    ...
    void communicateMaps(int _nCpu, std::vector<typename key_type::value_type>& localKeys, std::vector<int>& localRanks)
    {
      ...
      MPI_Allgatherv(
        &localKeys[0], localMapSize, MPI_UNSIGNED_LONG_LONG, &globalKeys[0], &mapSizes[0], &displacements[0], MPI_UNSIGNED_LONG_LONG, MPI_COMM_WORLD);
