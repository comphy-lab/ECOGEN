...
template <typename T, std::size_t N> class vector
{
  ...
  public: // ctors
    ...
    template <typename T2> vector(const vector<T2, N>& other)
    {
      for (unsigned int i = 0; i < N; ++i) data[i] = static_cast<T>(other.data[i]);
    }
...
template <class T, std::size_t N> bool operator==(const math::vector<T, N>& lhs, const math::vector<T, N>& rhs) { return lhs.data == rhs.data; }
...
template <typename T, std::size_t N> math::vector<T, N> operator-(math::vector<T, N> v)
{
...
template <typename U, std::size_t N> U norm2(const math::vector<U, N>& v) { return dot(v, v); }

template <typename U, std::size_t N> U norm_inf(const math::vector<U, N>& v)
{
  using namespace std;
  auto m = std::fabs(v[0]);
...
template <typename U, typename V, std::size_t N>
typename std::enable_if<N == 3, math::vector<typename std::common_type<U, V>::type, N>>::type cross(const math::vector<U, N>& lhs,
