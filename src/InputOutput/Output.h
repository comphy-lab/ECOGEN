...
class Output
{
  public:
    ...
    //! \brief   Main constructor for datasets used for OutputVTK and OutputGNU according to outputMode
    ...
    //! \brief   Constructor for datasets used for OutputVTK when mesh mapping restart option is activated
    //! \param   nameRun                       Folder to store results
    ...
    virtual void probeDisplacement(const double& /*dt*/) { throw ErrorECOGEN("probeDisplacement not available for requested output format"); };

    virtual void locateProbeInMesh(const TypeMeshContainer<Cell*>& /*cells*/, const int& /*nbCells*/, bool /*localSeeking*/ = false)
    {
      try {
        throw ErrorECOGEN("locateProbeInMesh not available for requested output format");
      }
      catch (ErrorECOGEN&) {
        throw;
      }
    ...
    {
      try {
        throw ErrorECOGEN("writeResults not available for requested output format");
      }
      catch (ErrorECOGEN&) {
        throw;
      }
    ...
    void printTree(Mesh* mesh, std::vector<Cell*>* cellsLvl, int m_resumeAMRsaveFreq);
    ...
    {
      try {
        throw ErrorECOGEN("initializeSpecificOutput not available for requested output format");
      }
      catch (ErrorECOGEN&) {
        throw;
      }
    ...
    //! \details Only the content since the file number to be resumed is kept (as the next files will overwrite the later content)
    void readInfos();
    //! \brief   Read and return the number of cpu from the infoCalcul file of a performed simulation
    int readNbCpu();
    //! \brief   Read results of a previous simulation to resume from it
    ...
    {
      try {
        throw ErrorECOGEN("readResults not available for requested output format");
      }
      catch (ErrorECOGEN&) {
        throw;
      }
    };
    //! \brief   Read results of a single partition of a previous simulation to resume from it
    ...
    {
      try {
        throw ErrorECOGEN("readResultsCpu not available for requested output format");
      }
      catch (ErrorECOGEN&) {
        throw;
      }
    ...
    {
      try {
        throw ErrorECOGEN("getNextTime not available for requested output format");
      }
      catch (ErrorECOGEN&) {
        throw;
      }
    ...
    {
      try {
        throw ErrorECOGEN("possesses not available for requested output format");
      }
      catch (ErrorECOGEN&) {
        throw;
      }
