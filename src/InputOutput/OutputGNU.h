...
class OutputGNU : public Output
{
  public:
    ...
    void initializeSpecificOutput() override {};                                                   // Nothing to do for this output
    void initializeSpecificOutput(std::vector<CellInterface*>* /*cellInterfacesLvl*/) override {}; // Nothing to do for this output
  ...
  protected:
    ...
    void writeScriptGnuplotPDF(const int& dim);
    ...
    void printBlocGnuplot(std::ofstream& fileStream, int& index, const int& dim, bool pause = true);

    std::string formatVarNameStyle(std::string const& strToFormat) const;

    std::string m_fileNameVisu;
    std::string m_fileNamePDF;
