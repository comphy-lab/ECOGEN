...
int stretchZone::verifyStretching(std::vector<stretchZone>& tabStretch, const double l, std::string fileName)
{
  ...
  try {
    ...
    for (unsigned int i = 1; i < tabStretch.size(); i++) {
      ...
      //AF//TODO// Precompute pow() or/and 1./...
      if (rankCpu == 0) {
        double dX0L((tabStretch[i - 1].m_endAt - tabStretch[i - 1].m_startAt) * (1. - tabStretch[i - 1].m_factor) /
                    (1. - std::pow(tabStretch[i - 1].m_factor, tabStretch[i - 1].m_numberCells)));
        double dXnL(dX0L * std::pow(tabStretch[i - 1].m_factor, tabStretch[i - 1].m_numberCells - 1));
        double dX0R((tabStretch[i].m_endAt - tabStretch[i].m_startAt) * (1. - tabStretch[i].m_factor) /
                    (1. - std::pow(tabStretch[i].m_factor, tabStretch[i].m_numberCells)));
        if (dXnL / dX0R > 1.1 || dXnL / dX0R < 0.9)
          std::cout << "WARNING: cell factor is " << dXnL / dX0R << " between zones " << i - 1 << " and " << i << std::endl;
      }
    }
  }
  catch (ErrorXML&) {
