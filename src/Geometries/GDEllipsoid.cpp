  ...
  GeometricalDomain(name, vecPhases, mixture, vecTransports, physicalEntity)
{
  ...
  case 'Z':
    ...
    break;
  default:
    throw ErrorXMLAttribut("axis3", fileName, __FILE__, __LINE__);
  }
  //Ellipsoid center
  double x(0.), y(0.), z(0.);
...
bool GDEllipsoid::belong(Coord& posElement, const int& /*lvl*/) const
{
  ...
  //AF//TODO//Remove pow call+axis.size() (ECOGEN_DISK_DIMENSION)+remove internal ifs?
  for (unsigned int i = 0; i < axes.size(); i++) {
    switch (axes[i]) {
    case X:
      if (i == 0) {
        sum += std::pow((posElement.getX() - m_centerPos.getX()) / m_radius1, 2.);
        break;
      }
      else if (i == 1) {
        sum += std::pow((posElement.getX() - m_centerPos.getX()) / m_radius2, 2.);
        break;
      }
      else {
        sum += std::pow((posElement.getX() - m_centerPos.getX()) / m_radius3, 2.);
        break;
      }
    case Y:
      if (i == 0) {
        sum += std::pow((posElement.getY() - m_centerPos.getY()) / m_radius1, 2.);
        break;
      }
      else if (i == 1) {
        sum += std::pow((posElement.getY() - m_centerPos.getY()) / m_radius2, 2.);
        break;
      }
      else {
        sum += std::pow((posElement.getY() - m_centerPos.getY()) / m_radius3, 2.);
        break;
      }
    case Z:
      if (i == 0) {
        sum += std::pow((posElement.getZ() - m_centerPos.getZ()) / m_radius1, 2.);
        break;
      }
      else if (i == 1) {
        sum += std::pow((posElement.getZ() - m_centerPos.getZ()) / m_radius2, 2.);
        break;
      }
      else {
        sum += std::pow((posElement.getZ() - m_centerPos.getZ()) / m_radius3, 2.);
        break;
      }
    }
  }
