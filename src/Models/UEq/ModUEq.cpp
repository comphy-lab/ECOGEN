...
{
  // Nullify pointers to avoid segfault when calling the destructor and when those buffers were not instantiated
  fluxBuff    = nullptr;
  fluxBuffMRF = nullptr;
}
...
void ModUEq::allocateCons(Flux** cons) { *cons = new FluxUEq(numberPhases); }

//***********************************************************************

void ModUEq::allocatePhase(Phase** phase) { *phase = new PhaseUEq; }

//***********************************************************************

void ModUEq::allocateMixture(Mixture** mixture) { *mixture = new MixUEq; }

//***********************************************************************

void ModUEq::allocatePhaseGradient(GradPhase** phase) { *phase = new GradPhaseUEq; }

//***********************************************************************

void ModUEq::allocateMixtureGradient(GradMixture** mixture) { *mixture = new GradMixUEq; }
...
void ModUEq::solveRiemannIntern(
  Cell& cellLeft, Cell& cellRight, const double& dxLeft, const double& dxRight, double& dtMax, std::vector<double>& boundData) const
{
  ...
  double uL = cellLeft.getMixture()->getVelocity().getX(), cL = cellLeft.getMixture()->getFrozenSoundSpeed(),
         pL = cellLeft.getMixture()->getPressure(), rhoL = cellLeft.getMixture()->getDensity();
  double uR = cellRight.getMixture()->getVelocity().getX(), cR = cellRight.getMixture()->getFrozenSoundSpeed(),
         pR = cellRight.getMixture()->getPressure(), rhoR = cellRight.getMixture()->getDensity();
  ...
  if (sL >= 0.) {
    for (int k = 0; k < numberPhases; k++) {
      phase          = cellLeft.getPhase(k);
      double alpha   = phase->getAlpha();
      double density = phase->getDensity();
      double energy  = phase->getEnergy();
    ...
    double vitY        = cellLeft.getMixture()->getVelocity().getY();
    double vitZ        = cellLeft.getMixture()->getVelocity().getZ();
    ...
    double vitY        = cellRight.getMixture()->getVelocity().getY();
    double vitZ        = cellRight.getMixture()->getVelocity().getZ();
...
void ModUEq::solveRiemannInternMRF(Cell& cellLeft,
                                   Cell& cellRight,
                                   const double& dxLeft,
...
void ModUEq::solveRiemannInletTank(Cell& cellLeft,
                                   const double& dxLeft,
                                   double& dtMax,
                                   const double* ak0,
                                   const double* rhok0,
                                   ...
                                   std::vector<double>& boundData) const
{
  ...
  double uL = cellLeft.getMixture()->getVelocity().getX(), cL = cellLeft.getMixture()->getWoodSoundSpeed(), pL = cellLeft.getMixture()->getPressure(),
         rhoL = cellLeft.getMixture()->getDensity();
  ...
  for (int k = 0; k < numberPhases; k++) {
    phase = cellLeft.getPhase(k);
    //TB->rhokStar[k] = TB->eos[k]->computeDensityIsentropic(phase->getPressure(), phase->getDensity(), pStar); //other possiblity
    TB->rhokStar[k] = TB->eos[k]->computeDensityHugoniot(phase->getPressure(), phase->getDensity(), pStar);
    TB->eos[k]->verifyAndCorrectDensityMax(TB->rhokStar[k]);
    vStar += phase->getAlpha() * phase->getDensity() / rhoL / std::max(TB->rhokStar[k], epsilonAlphaNull);
  }
  vmv0 = vStar - 1. / rhoL;
  ...
  if (sL >= 0.) { //supersonic outflow => left state solution
    ...
    for (int k = 0; k < numberPhases; k++) {
      phase           = cellLeft.getPhase(k);
      TB->rhokStar[k] = phase->getDensity();
      TB->eos[k]->verifyAndCorrectDensityMax(TB->rhokStar[k]);
      TB->YkStar[k] = phase->getAlpha() * phase->getDensity() / rhoL;
    }
    rhoStar = rhoL;
    uyStar  = uyL;
    uzStar  = uzL;
  ...
  //-------
    ...
    do {
      ...
      if (iteration > 50) {
        for (int i = 0; i < 50; i++) {
          std::cout << tabp[i] << " " << tabf[i] << std::endl;
        }
      ...
      for (int k = 0; k < numberPhases; k++) {
        phase = cellLeft.getPhase(k);
        rhok  = TB->eos[k]->computeDensityIsentropic(phase->getPressure(), phase->getDensity(), p, &drhok); //other possiblity
        //rhok = TB->eos[k]->computeDensityHugoniot(phase->getPressure(), phase->getDensity(), p, &drhok);
        TB->eos[k]->verifyAndCorrectDensityMax(rhok);
        YkL      = phase->getAlpha() * phase->getDensity() / rhoL;
  ...
  for (int k = 0; k < numberPhases; k++) {
    if (!m_lowMach) ek = TB->eos[k]->computeEnergy(TB->rhokStar[k], pStar);
      ...
      ek = cellLeft.getPhase(k)->getEnergy();
    }
...
void ModUEq::solveRiemannInletInjStagState(Cell& cellLeft,
                                           const double& dxLeft,
                                           double& dtMax,
                                           const double m0,
                                           const double* ak0,
                                           ...
                                           std::vector<double>& boundData) const
{
  ...
  //-----------------
    ...
    do {
      ...
      iteration++;
      if (iteration > 50) {
        Errors::errorMessage("solveRiemannInletInjStagState not converged in ModUEq");
        exit(0);
      }
      //Physical pressure ?
      for (int k = 0; k < numberPhases; k++) {
...
void ModUEq::solveRiemannInletInjTemp(
  Cell& cellLeft, const double& dxLeft, double& dtMax, const double m0, const double* Tk0, const double* ak0, std::vector<double>& boundData) const
{
  double sL, zL, sM;

  double uL   = cellLeft.getMixture()->getVelocity().getX();
  double vL   = cellLeft.getMixture()->getVelocity().getY();
  double wL   = cellLeft.getMixture()->getVelocity().getZ();
  ...
  double u(0.), du(0.);
  double rhokStar(0.), drhokStar(0.), vStarL(0.), dvStarL(0.), vSmvL(0.), mL(zL), dmL(0.), YkL(0.);
  ...
  do {
    ...
    it++;
    if (it > 50) {
      Errors::errorMessage("solveRiemannInletInjTemp not converged in ModUEq");
    }
    ...
    for (int k = 0; k < numberPhases; k++) {
      phase    = cellLeft.getPhase(k);
      rhokStar = TB->eos[k]->computeDensityIsentropic(phase->getPressure(), phase->getDensity(), pStar, &drhokStar); //other possiblity
      // rhok = TB->eos[k]->computeDensityHugoniot(phase->getPressure(), phase->getDensity(), pStar, &drhok);
      YkL = phase->getAlpha() * phase->getDensity() / rhoL;
      if (k == 0) YkL = 1.;
      else YkL = 0.;
      vStarL  += YkL / std::max(rhokStar, epsilonAlphaNull);
      dvStarL -= YkL / std::max((rhokStar * rhokStar), epsilonAlphaNull) * drhokStar;
    }

    vSmvL = vStarL - 1. / rhoL;
    if (std::fabs(vSmvL) > 1.e-10) {
      mL  = std::sqrt((pL - pStar) / vSmvL);
      dmL = 0.5 * (-vSmvL + (pStar - pL) * dvStarL) / (vSmvL * vSmvL) / mL;
    }
    else {
      mL  = zL;
      dmL = 0.;
  ...
  //Flux completion
  double Estar(0.5 * (uStar * uStar + vL * vL + wL * wL)), ekStar;
  for (int k = 0; k < numberPhases; k++) {
    ...
    if (!m_lowMach) ekStar = TB->eos[k]->computeEnergy(TB->rhokStar[k], pStar);
      ...
      ekStar = cellLeft.getPhase(k)->getEnergy();
    }
  ...
  static_cast<FluxUEq*>(fluxBuff)->m_momentum.setX(uStar * uStar * rhoStar + pStar);
  static_cast<FluxUEq*>(fluxBuff)->m_momentum.setY(rhoStar * uStar * vL);
  static_cast<FluxUEq*>(fluxBuff)->m_momentum.setZ(rhoStar * uStar * wL);
  static_cast<FluxUEq*>(fluxBuff)->m_energMixture = (Estar * rhoStar + pStar) * uStar;
...
void ModUEq::solveRiemannOutletPressure(Cell& cellLeft, const double& dxLeft, double& dtMax, const double p0, std::vector<double>& boundData) const
{
  ...
  double vSmvL, mL(zL);
  for (int k = 0; k < numberPhases; k++) {
    //TB->rhokStar[k] = TB->eos[k]->computeDensityIsentropic(pL, phase->getDensity(), pStar); //other possiblity
  ...
  if (sL >= 0.) { //Supersonic outflow => Left state solution
    ...
    pStar = pL;
    for (int k = 0; k < numberPhases; k++) {
      TB->rhokStar[k] = cellLeft.getPhase(k)->getDensity();
    }
    vStar = 1. / rhoL;
  }
...
void ModUEq::solveRiemannOutletMassflow(Cell& cellLeft, const double& dxLeft, double& dtMax, const double m0, std::vector<double>& boundData) const
{
  ...
  do {
    ...
    for (int k = 0; k < numberPhases; k++) {
      phase = cellLeft.getPhase(k);
      //TB->rhokStar[k] = TB->eos[k]->computeDensityIsentropic(phase->getPressure(), phase->getDensity(), pStar, &drhokStar); //other possiblity
      TB->rhokStar[k] = TB->eos[k]->computeDensityHugoniot(phase->getPressure(), phase->getDensity(), p, &drhokStar);
      TB->eos[k]->verifyAndCorrectDensityMax(TB->rhokStar[k]);
      YkL     = phase->getAlpha() * phase->getDensity() / rhoL;
...
void ModUEq::solveRiemannTransportIntern(Cell& cellLeft, Cell& cellRight)
{
  for (int k = 0; k < numberTransports; k++) {
    fluxBufferTransport[k].solveRiemann(cellLeft.getTransport(k).getValue(),
                                        cellRight.getTransport(k).getValue(),
                                        static_cast<FluxUEq*>(fluxBuff)->m_sM);
  }
}
...
{
  for (int k = 0; k < numberTransports; k++) {
    fluxBufferTransport[k].solveRiemannInletInjStagState(cellLeft.getTransport(k).getValue(),
                                                         static_cast<FluxUEq*>(fluxBuff)->m_sM,
                                                         valueTransports[k]);
  }
}
...
const double& ModUEq::getSM() { return static_cast<FluxUEq*>(fluxBuff)->m_sM; }
...
void ModUEq::addNonConsMrfFlux(Phase** phases) { static_cast<FluxUEq*>(fluxBuffMRF)->addNonConsMrfFlux(phases); }
...
{
  Coord projectedFlux;
  projectedFlux.setX(normal.getX() * static_cast<FluxUEq*>(fluxBuffMRF)->m_momentum.getX() +
                     tangent.getX() * static_cast<FluxUEq*>(fluxBuffMRF)->m_momentum.getY() +
                     binormal.getX() * static_cast<FluxUEq*>(fluxBuffMRF)->m_momentum.getZ());
  projectedFlux.setY(normal.getY() * static_cast<FluxUEq*>(fluxBuffMRF)->m_momentum.getX() +
                     tangent.getY() * static_cast<FluxUEq*>(fluxBuffMRF)->m_momentum.getY() +
                     binormal.getY() * static_cast<FluxUEq*>(fluxBuffMRF)->m_momentum.getZ());
  projectedFlux.setZ(normal.getZ() * static_cast<FluxUEq*>(fluxBuffMRF)->m_momentum.getX() +
                     tangent.getZ() * static_cast<FluxUEq*>(fluxBuffMRF)->m_momentum.getY() +
                     binormal.getZ() * static_cast<FluxUEq*>(fluxBuffMRF)->m_momentum.getZ());
