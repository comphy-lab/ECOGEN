...
void RelaxationPT::relaxation(Cell* cell, const double& /*dt*/, Prim type)
{
  ...
  if (relax) {
    ...
    for (int k = 0; k < numberPhases; k++) {
      if (cell->getPhase(k, type)->getEos()->getType() != TypeEOS::IG && cell->getPhase(k, type)->getEos()->getType() != TypeEOS::SG) {
        Errors::errorMessage("Only IG and SG permitted in RelaxationPT::relaxation");
      }
    }
...
double RelaxationPT::analyticalPressure(Cell* cell, Prim type) const
{
  ...
  for (int k = 0; k < numberPhases; k++) {
    if (cell->getPhase(k)->getEos()->getType() != TypeEOS::IG && cell->getPhase(k)->getEos()->getType() != TypeEOS::SG) {
      Errors::errorMessage("Only IG and SG permitted in RelaxationPT::analyticalPressure");
    }
  }
