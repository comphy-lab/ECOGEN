...
CellO2NS::~CellO2NS()
{
  if (m_gradPhase) { // when the code fails before model construction, m_gradPhase may not be allocated while numberPhases is already set
    for (int k = 0; k < numberPhases; k++) {
      if (m_gradPhase[k]) delete m_gradPhase[k];
    }
  }
  delete[] m_gradPhase;
...
void CellO2NS::limitGradientsO2(Limiter& globalLimiter)
{
  ...
  for (unsigned int i = 0; i < m_cellInterfaces.size(); i++) {
    ...
    for (int t = 0; t < numberTransports; t++) {
      slopesTransportLocal2[t] = std::min(
        slopesTransportLocal2[t],
        globalLimiter.computeGradientLimiter(this->getTransport(t).getValue(), buffTransportMin[t], buffTransportMax[t], slopesTransportLocal1[t]));
    }
  }
...
GradPhase* CellO2NS::getGradPhase(const int& phaseNumber) const { return m_gradPhase[phaseNumber]; }

//***********************************************************************

GradMixture* CellO2NS::getGradMixture() const { return m_gradMixture; }

//***********************************************************************

GradTransport* CellO2NS::getGradTransport(const int& transportNumber) const { return &m_gradTransport[transportNumber]; }
