...
void FluxUEqTotEnergy::printFlux() const { std::cout << m_mass << " " << m_momentum.getX() << " " << m_totEnerg << std::endl; }
...
void FluxUEqTotEnergy::setBufferFlux(Cell& cell) { static_cast<FluxUEqTotEnergy*>(fluxBuff)->buildCons(cell.getPhases(), cell.getMixture()); }
...
void FluxUEqTotEnergy::addNonCons(double coefA, const Cell* cell, const Coord& /*normal*/, const Coord& /*tangent*/, const Coord& /*binormal*/)
{
  ...
  for (int k = 0; k < numberPhases; k++) {
    ...
    m_totEnerg[k] += coefA * (phase->getY() * (pStar * uStar - pMix * uStar) - static_cast<FluxUEqTotEnergy*>(fluxBuff)->m_alphap[k] * uStar +
                              phase->getAlpha() * phase->getPressure() * uStar);
  }
}
