...
GDHalfSpace::GDHalfSpace(std::string name,
  ...
  GeometricalDomain(name, vecPhases, mixture, vecTransports, physicalEntity)
{
  ...
  case 'Z':
    ...
    break;
  default:
    throw ErrorXMLAttribut("axis", fileName, __FILE__, __LINE__);
  }
  //Direction
  ...
  if (direction == "POSITIVE") {
    ...
    throw ErrorXMLAttribut("direction", fileName, __FILE__, __LINE__);
  }
...
{
  bool result(false);
  //AF//TODO// Simplification ?
  switch (m_axis) {
  case X:
    if (m_direction >= 0) {
      if (posElement.getX() >= m_position) result = true;
    }
    else {
      if (posElement.getX() <= m_position) result = true;
    }
  ...
  case Y:
    if (m_direction >= 0) {
      if (posElement.getY() >= m_position) result = true;
    }
    else {
      if (posElement.getY() <= m_position) result = true;
    }
  ...
  case Z:
    if (m_direction >= 0) {
      if (posElement.getZ() >= m_position) result = true;
    }
    else {
      if (posElement.getZ() <= m_position) result = true;
    }
