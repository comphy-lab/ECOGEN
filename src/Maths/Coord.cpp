...
Coord::Coord(const double& x, const double& y, const double& z) : m_x(x), m_y(y), m_z(z) {}
...
double Coord::norm() const { return sqrt(m_x * m_x + m_y * m_y + m_z * m_z); }

//*********************************************************************

double Coord::squaredNorm() const { return m_x * m_x + m_y * m_y + m_z * m_z; }
...
double Coord::scalar(const Coord& a) const { return m_x * a.m_x + m_y * a.m_y + m_z * a.m_z; }
...
double Coord::scalarProduct(const Coord& v1, const Coord& v2) { return v1.m_x * v2.m_x + v1.m_y * v2.m_y + v1.m_z * v2.m_z; }
...
void Coord::normalized() { *this = *this / this->norm(); }
...
double Coord::cos(const Coord& v1, const Coord& v2)
{
  ...
  if (nv1 > 1e-6 && nv2 > 1e-6) {
    cos = v1.scalar(v2) / (nv1 * nv2);
  }
  return cos;
}
...
void Coord::printInfo() const { std::cout << " X = " << m_x << " Y = " << m_y << " Z = " << m_z << std::endl; }
