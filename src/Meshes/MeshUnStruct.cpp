  ...
  m_totalVolume(0.)
{
  ...
  if (!meshFileStream) {
    throw ErrorECOGEN("mesh file not found : " + m_meshFile, __FILE__, __LINE__);
  }
...
MeshUnStruct::~MeshUnStruct()
{
  ...
  for (unsigned int l = 0; l < m_bound.size(); l++) {
    delete m_bound[l];
  }
}
                                      ...
                                      std::string ordreCalcul)
{
  try {
    if (Ncpu == 1) {
      this->initGeometryMonoCPU(cells, cellInterfaces, ordreCalcul);
    }
    else {
      // Preprocessing of mesh file with CPU 0
      if (pretraitementParallele) {
        if (rankCpu == 0) {
          this->preProcessMeshFileForParallel();
        }
...
void MeshUnStruct::writeMeshInfoData() const
{
  ...
  if (m_numberElements0D != 0) {
    ...
    std::cout << "    ~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
    if (m_numberPoints != 0) {
      std::cout << "      - number of vertices: " << m_numberPoints << std::endl;
    }
  }
  if (m_numberElements1D != 0) {
    ...
    if (m_numberSegments != 0) {
      std::cout << "      - number of segments: " << m_numberSegments << std::endl;
    }
  }
  if (m_numberElements2D != 0) {
    ...
    std::cout << "    ~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
    if (m_numberTriangles != 0) {
      std::cout << "      - number of triangles: " << m_numberTriangles << std::endl;
    }
    if (m_numberQuadrangles != 0) {
      std::cout << "      - number of quadrangles: " << m_numberQuadrangles << std::endl;
    }
  ...
  if (m_numberElements3D != 0) {
    ...
    std::cout << "    ~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
    if (m_numberTetrahedrons != 0) {
      std::cout << "      - number of tetrahedrons: " << m_numberTetrahedrons << std::endl;
    }
    if (m_numberPyramids != 0) {
      std::cout << "      - number of pyramids: " << m_numberPyramids << std::endl;
    }
    if (m_numberHexahedrons != 0) {
      std::cout << "      - number of hexahedrons: " << m_numberHexahedrons << std::endl;
    }
...
{
  fileStream << "    <Piece NumberOfPoints=\"" << m_numberNodes << "\" NumberOfCells=\"" << m_numberCellsCalcul - m_numberGhostCells << "\">"
             << std::endl;
}
    ...
    if (!m_elements[i]->isFantome()) {
      ...
      if (var > 0) { //We want to get the scalar data
        if (phase >= 0) {
          ...
          Errors::errorMessage("MeshUnStruct::getData: unknown number of phase: ", phase);
        }
      }
      else {
  ...
  for (int i = m_numberBoundFaces; i < m_numberInnerElements; i++) {
    if (!m_elements[i]->isFantome()) {
      ...
      if (var > 0) { //Scalars data are first set
        if (phase >= 0) {
          //phases data
          cellsLvl[0][numCell]->getPhase(phase)->setScalar(var, dataset[iterDataSet++]);
        }
        else if (phase == -1) {
          //mixture data
          cellsLvl[0][numCell]->getMixture()->setScalar(var, dataset[iterDataSet++]);
        }
        else if (phase == -2) {
          //transport data
          cellsLvl[0][numCell]->getTransport(var - 1).setValue(dataset[iterDataSet++]);
        }
        else {
          Errors::errorMessage("MeshUnStruct::setDataSet: unknown phase number: ", phase);
        }
      }
      else {
        //We want to get the vector data
