                                  ...
                                  Prim type)
{
  ...
  case vecPhases:
    if (m_cellLeft->getLvl() == m_cellRight->getLvl()) {
      ...
      //CoefAMR = 1 pour la gauche et on n'ajoute rien sur la right cell
      this->subtractFlux(1.); //Subtract of flux on left cell
    }
    else {
      //CoefAMR = 1 pour la droite et on ne retire rien sur la left cell
      this->addFlux(1.); //Add of flux on right cell
    }
    break;
  ...
  case vecPhasesO2:
    if (m_cellLeft->getLvl() == m_cellRight->getLvl()) {
      ...
      //CoefAMR = 1 pour la gauche et 0.5 pour la droite
      this->addFlux(0.5);     //Add of flux on right cell
      this->subtractFlux(1.); //Subtract of flux on left cell
    }
    else {
      //CoefAMR = 1 pour la droite et 0.5 pour la gauche
      this->addFlux(1.);       //Add of flux on right cell
