...
class EosIG : public Eos
{
  public:
    ...
    //! \param     pressure           pressure (p)
    //! \return    interfaceSoundSpeed
    //! \details   with : \f$  c(pI, p, \rho)  = \sqrt{  \frac{(\gamma-1) \ p_I + p}{\rho}} \f$
    ...
    //! \details   with : \f$  z(p, \rho)  = \sqrt{ \rho \ \gamma \ p } \f$
    ...
    //! \param     pressure           pressure (p)
    //! \return    rhoCISquare
    //! \details   with : \f$  z(p, \rho)  = (\gamma-1) \ p_I + p \f$
    double
    computeDensityTimesInterfaceSoundSpeedSquare(const double& /*density*/, const double& interfacePressure, const double& pressure) const override;
    ...
    //! \details   with finalDensity : \f$ \rho_f = \rho_i \left( \frac{p_f}{ p_i}\right) ^\frac{1}{\gamma} \f$ AND with   *drhodp :  \f$  \left.  \frac{\partial \rho}{\partial p} \right)_s   = \frac{\rho_f}{ \gamma \ p_f}  \f$
    ...
    //! \return    finalDensity  AND  *drhodp (OPTIONAL)
    //! \details   with finalDensity : \f$ \rho_f =\rho_i\frac{ (\gamma + 1)*p_f + (\gamma - 1)*p_i}{(\gamma - 1)p_f +(\gamma + 1)p_i) }\f$ AND with  *drhodp :  \f$  \left.  \frac{\partial \rho}{\partial p} \right)_Hug   =  \frac{4\gamma \rho_i \ p_i}{ \left( (\gamma - 1)p_f +(\gamma + 1)p_i \right) ^2}   \f$
    ...
    //! \param     finalPressure      final pressure (\f$ p_f \f$)
    //! \return    finalDensity  AND  *drhodp (OPTIONAL)
    //! \details   with finalDensity : \f$ \rho_f = \rho_i \frac{\gamma p_f}{\gamma p_f + p_i - p_f} \f$ AND with *drhodp : \f$ \frac{d\rho}{dp} = \rho_i \frac{\gamma p_i}{\left(\gamma p_f + p_i - p_f\right)^2} \f$
    double
    ...
    //! \details   with  rho : \f$  \rho(p, T_{sat})  =  \frac{p}{T_{sat} \ c_v \ (\gamma-1)}\ \f$ AND \f$\frac{d \rho}{d p} = \left.  \frac{\partial \rho}{\partial p} \right)_{T_{sat}} + \left.  \frac{\partial \rho}{\partial T_{sat} } \right)_p \frac{d T_{sat}}{d p}  =\frac{(\gamma-1)c_v\left(T_{sat} - p\frac{d T_{sat}}{d p}\right) }{\left((\gamma-1)c_vT_{sat}\right)^2 }\f$
    ...
    //! \details  with  rhoe : \f$  \rho\epsilon(p, \rho)  =  \frac{p}{(\gamma-1)} +\rho\epsilon_{ref}\f$ AND \f$ \frac{d \rho\epsilon}{d p} =\left.  \frac{\partial \rho\epsilon}{\partial p} \right)_{\rho} + \left.  \frac{\partial \rho\epsilon}{\partial\rho  } \right)_p \frac{d \rho}{d p}  = \frac{1}{(\gamma-1)} +\frac{d \rho}{d p}\epsilon_{ref} \f$
    ...
    //! \param  temperature temperature (T)
    //! \return derivative : \f$ \left. \frac{\partial \rho}{\partial p} \right)_T = \frac{1}{c_v (\gamma - 1) T} \f$
    ...
    //! \return \f$ \ "IG" \f$
    TypeEOS getType() const override { return TypeEOS::IG; };
