...
void Parallel::setNeighbour(const int neighbour) { m_isNeighbour[neighbour] = true; }
...
void Parallel::addSlopesToSend(int neighbour) { m_numberSlopesToSendToNeighbour[neighbour] += 1; }

//***********************************************************************

void Parallel::addSlopesToReceive(int neighbour) { m_numberSlopesToReceiveFromNeighbour[neighbour] += 1; }

//***********************************************************************

void Parallel::deleteSlopesToSend(int neighbour) { m_numberSlopesToSendToNeighbour[neighbour] -= 1; }

//***********************************************************************

void Parallel::deleteSlopesToReceive(int neighbour) { m_numberSlopesToReceiveFromNeighbour[neighbour] -= 1; }
...
TypeMeshContainer<Cell*>& Parallel::getElementsToSend(int neighbour) { return m_elementsToSend[neighbour]; }

//***********************************************************************

TypeMeshContainer<Cell*>& Parallel::getElementsToReceive(int neighbour) { return m_elementsToReceive[neighbour]; }
...
void Parallel::finalize(const int& lvlMax)
{
  if (Ncpu > 1) {
    ...
    destroy_array(m_isNeighbour);
    destroy_array(m_numberElementsToSendToNeighbour);
    destroy_array(m_numberElementsToReceiveFromNeighbour);
    destroy_array(m_numberSlopesToSendToNeighbour);
    destroy_array(m_numberSlopesToReceiveFromNeighbour);

    for (TypeMeshContainer<Cell*>& neighbour : m_elementsToSend) {
      neighbour.clear();
    }
    for (TypeMeshContainer<Cell*>& neighbour : m_elementsToReceive) {
      neighbour.clear();
    }
...
void Parallel::finalizePersistentCommunicationsPrimitives(const int& lvlMax)
{
  for (int lvl = 0; lvl <= lvlMax; lvl++) {
    if (m_isNeighbour) { // For some unexpected failure, m_isNeighbour may not have been allocated
      for (int neighbour = 0; neighbour < Ncpu; neighbour++) {
        if (m_isNeighbour[neighbour]) {
      ...
      // next arrays are allocated iff m_isNeighbour is
...
void Parallel::initializePersistentCommunicationsSlopes()
{
  for (int neighbour = 0; neighbour < Ncpu; neighbour++) {
    if (m_isNeighbour[neighbour]) {
      ...
      MPI_Recv_init(
        m_bufferReceiveSlopes[0][neighbour], numberReceive, MPI_DOUBLE, neighbour, rankCpu, MPI_COMM_WORLD, m_reqReceiveSlopes[0][neighbour]);
    }
  }
}
...
void Parallel::initializePersistentCommunicationsVector(const int& dim)
{
  for (int neighbour = 0; neighbour < Ncpu; neighbour++) {
    if (m_isNeighbour[neighbour]) {
      ...
      MPI_Recv_init(
        m_bufferReceiveVector[0][neighbour], numberReceive, MPI_DOUBLE, neighbour, rankCpu, MPI_COMM_WORLD, m_reqReceiveVector[0][neighbour]);
    }
  }
}
...
void Parallel::initializePersistentCommunicationsTransports()
{
  for (int neighbour = 0; neighbour < Ncpu; neighbour++) {
    if (m_isNeighbour[neighbour]) {
      ...
      MPI_Send_init(
        m_bufferSendTransports[0][neighbour], numberSend, MPI_DOUBLE, neighbour, neighbour, MPI_COMM_WORLD, m_reqSendTransports[0][neighbour]);

      //New receiving request and its associated buffer
      m_reqReceiveTransports[0][neighbour]    = new MPI_Request;
      m_bufferReceiveTransports[0][neighbour] = new double[numberReceive];
      MPI_Recv_init(
        m_bufferReceiveTransports[0][neighbour], numberReceive, MPI_DOUBLE, neighbour, rankCpu, MPI_COMM_WORLD, m_reqReceiveTransports[0][neighbour]);
    }
  }
}
...
void Parallel::initializePersistentCommunicationsAMR(
  const int& numberPrimitiveVariables, const int& numberSlopeVariables, const int& numberTransportVariables, const int& dim, const int& lvlMax)
{
  if (Ncpu > 1) {
    m_numberPrimitiveVariables = numberPrimitiveVariables;
    m_numberSlopeVariables     = numberSlopeVariables;
    ...
    //Initialization of communications for levels strictly superior to 0
    parallel.initializePersistentCommunicationsLvlAMR(lvlMax);
    //Initialization of communications for levels superior or equals to 0 (communications used only by AMR)
    parallel.initializePersistentCommunicationsFromLvl0AMR(lvlMax);
  }
  ...
  for (int lvl = 1; lvl <= lvlMax; lvl++) {
    for (int neighbour = 0; neighbour < Ncpu; neighbour++) {
      if (m_isNeighbour[neighbour]) {
        ...
        MPI_Send_init(
          m_bufferSendSlopes[lvl][neighbour], numberSend, MPI_DOUBLE, neighbour, neighbour, MPI_COMM_WORLD, m_reqSendSlopes[lvl][neighbour]);

        //New receiving request and its associated buffer
        m_reqReceiveSlopes[lvl][neighbour]    = new MPI_Request;
        m_bufferReceiveSlopes[lvl][neighbour] = new double[numberReceive];
        MPI_Recv_init(
          m_bufferReceiveSlopes[lvl][neighbour], numberReceive, MPI_DOUBLE, neighbour, rankCpu, MPI_COMM_WORLD, m_reqReceiveSlopes[lvl][neighbour]);
        ...
        MPI_Send_init(
          m_bufferSendVector[lvl][neighbour], numberSend, MPI_DOUBLE, neighbour, neighbour, MPI_COMM_WORLD, m_reqSendVector[lvl][neighbour]);

        //New receiving request and its associated buffer
        m_reqReceiveVector[lvl][neighbour]    = new MPI_Request;
        m_bufferReceiveVector[lvl][neighbour] = new double[numberReceive];
        MPI_Recv_init(
          m_bufferReceiveVector[lvl][neighbour], numberReceive, MPI_DOUBLE, neighbour, rankCpu, MPI_COMM_WORLD, m_reqReceiveVector[lvl][neighbour]);
        ...
        MPI_Send_init(
          m_bufferSendTransports[lvl][neighbour], numberSend, MPI_DOUBLE, neighbour, neighbour, MPI_COMM_WORLD, m_reqSendTransports[lvl][neighbour]);

        //New receiving request and its associated buffer
        m_reqReceiveTransports[lvl][neighbour]    = new MPI_Request;
        m_bufferReceiveTransports[lvl][neighbour] = new double[numberReceive];
        MPI_Recv_init(m_bufferReceiveTransports[lvl][neighbour],
                      numberReceive,
                      MPI_DOUBLE,
                      neighbour,
                      rankCpu,
                      MPI_COMM_WORLD,
                      m_reqReceiveTransports[lvl][neighbour]);
      }
    }
  }
}

void Parallel::initializePersistentCommunicationsFromLvl0AMR(const int& lvlMax)
{
  //Parallel variables used only by AMR: We need to initialize them starting from 0 as it cannot be done in the 'initialize' method
  //(otherwise it creates a memory leak).
  // Variables needed only for AMR: not initialized at level 0 because they are useless and
  // will never be deleted if allocated in 'initialization'
  for (int lvl = 0; lvl <= lvlMax; lvl++) {
    ...
    const int number{lvl == 0 ? 1 : 0}; // initialize both sending and receiving arrays at 0 size for AMR levels > 0
                                        // and with 1 variable per element for level 0.
    //Xi variable
    //-----------
    this->initializePersistentCommunicationsXi(lvl, number);

    //Split variable
    //--------------
    this->initializePersistentCommunicationsSplit(lvl, number);
  }
}
...
void Parallel::clearRequestsAndBuffers(int lvl)
{
  for (int neighbour = 0; neighbour < Ncpu; neighbour++) {
    if (m_reqSend[lvl][neighbour] != NULL) {
      ...
      destroy(m_reqSend[lvl][neighbour]);
      destroy(m_reqReceive[lvl][neighbour]);
      destroy(m_reqSendSlopes[lvl][neighbour]);
      destroy(m_reqReceiveSlopes[lvl][neighbour]);
      destroy(m_reqSendVector[lvl][neighbour]);
      destroy(m_reqReceiveVector[lvl][neighbour]);
      destroy(m_reqSendTransports[lvl][neighbour]);
      destroy(m_reqReceiveTransports[lvl][neighbour]);
      destroy(m_reqSendXi[lvl][neighbour]);
      destroy(m_reqReceiveXi[lvl][neighbour]);
      destroy(m_reqSendSplit[lvl][neighbour]);
      destroy(m_reqReceiveSplit[lvl][neighbour]);

      destroy_array(m_bufferSend[lvl][neighbour]);
      destroy_array(m_bufferReceive[lvl][neighbour]);
      destroy_array(m_bufferSendSlopes[lvl][neighbour]);
      destroy_array(m_bufferReceiveSlopes[lvl][neighbour]);
      destroy_array(m_bufferSendVector[lvl][neighbour]);
      destroy_array(m_bufferReceiveVector[lvl][neighbour]);
      destroy_array(m_bufferSendTransports[lvl][neighbour]);
      destroy_array(m_bufferReceiveTransports[lvl][neighbour]);
      destroy_array(m_bufferSendXi[lvl][neighbour]);
      destroy_array(m_bufferReceiveXi[lvl][neighbour]);
      destroy_array(m_bufferSendSplit[lvl][neighbour]);
      destroy_array(m_bufferReceiveSplit[lvl][neighbour]);
    }
  }
}
...
void Parallel::updatePersistentCommunicationsAMR(const int& dim)
{
  ...
  const int rootLvl{0};
  this->clearRequestsAndBuffers(rootLvl);
  ...
  parallel.initializePersistentCommunicationsTransports();
  //Initialization of communications for AMR variables
  const int number{1}; // to initialize both sending and receiving arrays to transfer 1 variable per element to transfer
  parallel.initializePersistentCommunicationsXi(rootLvl, number);
  parallel.initializePersistentCommunicationsSplit(rootLvl, number);
...
void Parallel::updatePersistentCommunicationsLvlAMR(int lvl, const int& dim)
{
  ...
  for (int neighbour = 0; neighbour < Ncpu; neighbour++) {
    if (m_isNeighbour[neighbour]) {
      ...
      MPI_Recv_init(
        m_bufferReceiveSplit[lvl][neighbour], numberReceive, MPI_C_BOOL, neighbour, rankCpu, MPI_COMM_WORLD, m_reqReceiveSplit[lvl][neighbour]);
    }
  }
}
...
void Parallel::initializePersistentCommunicationsXi(const int& lvl, const int& number)
{
...
void Parallel::initializePersistentCommunicationsSplit(const int& lvl, const int& number)
{
...
{
  for (int neighbour = 0; neighbour < Ncpu; neighbour++) {
    //AF//Q What is the overhead of persistent comm initialization? Is it better to initialize comm for all ranks (no need to
    //update after AMR load balancing then) or to initialize them only for neighbouring ranks (it is needed to update the comms after load balancing
    //in this case)
    ...
    MPI_Send_init(&m_bufferNumberElementsToSendToNeighbor[neighbour],
                  numberSend,
                  MPI_INT,
                  neighbour,
                  neighbour,
                  MPI_COMM_WORLD,
                  m_reqNumberElementsToSendToNeighbor[neighbour]);
    ...
    MPI_Recv_init(&m_bufferNumberElementsToReceiveFromNeighbour[neighbour],
                  numberReceive,
                  MPI_INT,
                  neighbour,
                  rankCpu,
                  MPI_COMM_WORLD,
                  m_reqNumberElementsToReceiveFromNeighbour[neighbour]);

    //New sending request and its associated buffer
    m_reqNumberSlopesToSendToNeighbor[neighbour]    = new MPI_Request;
    m_bufferNumberSlopesToSendToNeighbor[neighbour] = 0;
    MPI_Send_init(&m_bufferNumberSlopesToSendToNeighbor[neighbour],
                  numberSend,
                  MPI_INT,
                  neighbour,
                  neighbour,
                  MPI_COMM_WORLD,
                  m_reqNumberSlopesToSendToNeighbor[neighbour]);

    //New receiving request and its associated buffer
    m_reqNumberSlopesToReceiveFromNeighbour[neighbour]    = new MPI_Request;
    m_bufferNumberSlopesToReceiveFromNeighbour[neighbour] = 0;
    MPI_Recv_init(&m_bufferNumberSlopesToReceiveFromNeighbour[neighbour],
                  numberReceive,
                  MPI_INT,
                  neighbour,
                  rankCpu,
                  MPI_COMM_WORLD,
                  m_reqNumberSlopesToReceiveFromNeighbour[neighbour]);
  }
}
  ...
  for (int neighbour = 0; neighbour < Ncpu; neighbour++) {
    if (m_isNeighbour[neighbour]) {
      ...
      for (int i = 0; i < m_numberElementsToSendToNeighbour[neighbour]; i++) {
        ...
        m_elementsToSend[neighbour][i]->fillNumberElementsToSendToNeighbour(
          m_bufferNumberElementsToSendToNeighbor[neighbour], m_bufferNumberSlopesToSendToNeighbor[neighbour], lvl, neighbour, 0);
      }
