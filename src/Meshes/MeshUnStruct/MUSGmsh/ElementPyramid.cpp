...
ElementPyramid::ElementPyramid() : ElementNS(TYPEGMSH, NUMBERNODES, NUMBERFACES, TYPEVTK) {}
...
{
  //AF//TODO// /6 once only or directly compute vol*6
  // The volume is computed using the two tetrahedrons included in the pyramid
  Coord v1, v2, v3;
  v1.setFromSubtractedVectors(nodes[4], nodes[1]);
  v2.setFromSubtractedVectors(nodes[4], nodes[0]);
  v3.setFromSubtractedVectors(nodes[4], nodes[2]);
  double volumeT1 = std::fabs(Coord::determinant(v1, v2, v3)) / 6.; // Tetrahedron's volume
  v1.setFromSubtractedVectors(nodes[4], nodes[2]);
  v2.setFromSubtractedVectors(nodes[4], nodes[3]);
  v3.setFromSubtractedVectors(nodes[4], nodes[0]);
