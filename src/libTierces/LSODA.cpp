/*
 * HISTORY:
 * This is a CPP version of the LSODA library slightly modified by
 * Kevin Schmidmayer.
 * The original CPP version was aquired from
 * https://github.com/dilawar/libsoda, which was also acquired from
 * http://www.ccl.net/cca/software/SOURCES/C/kinetics2/index.shtml and modified
 * by Heng Li <lh3lh3@gmail.com>. Heng merged several C files into one and added
 * a simpler interface. [Available here]
 * (http://lh3lh3.users.sourceforge.net/download/lsoda.c)

 * The original source code came with no license or copyright
 * information. Heng Li released his modification under the MIT/X11 license.
 * Dilawar Singh <dilawars@ncbs.res.in> maintained the same license and removed
 * quite a lot of text/comments from this library.
...
{
    if (illin == 5) {
        errors.push_back(Errors("[lsoda] repeated occurrence of illegal input. run aborted.. apparent infinite loop.", __FILE__, __LINE__));
    }
    else
    {
        illin++;
                  ...
                  void *_data)
{
    ...
    */
    ...
    {
        errors.push_back(Errors("[lsoda] illegal istate = " + to_string(*istate), __FILE__, __LINE__));
        terminate(istate);
        return;
    }
    if (itask < 1 || itask > 5)
    {
        errors.push_back(Errors("[lsoda] illegal itask = " + to_string(itask), __FILE__, __LINE__));
        terminate(istate);
        return;
    }
    if (init == 0 && (*istate == 2 || *istate == 3))
    {
        errors.push_back(Errors("[lsoda] istate > 1 but lsoda not initialized", __FILE__, __LINE__));
        terminate(istate);
        return;
    }
    ...
    */
    ...
    if (*istate == 1 || *istate == 3)
    {
        ...
        if (neq <= 0)
        {
            errors.push_back(Errors("[lsoda] neq = " + to_string(neq) + " is less than 1.", __FILE__, __LINE__));
            terminate(istate);
            return;
        }
        if (*istate == 3 && neq > n)
        {
            errors.push_back(Errors("[lsoda] istate = 3 and neq increased", __FILE__, __LINE__));
            terminate(istate);
            return;
        }
        n = neq;
        if (itol_ < 1 || itol_ > 4)
        {
            errors.push_back(Errors("[lsoda] itol = " + to_string(itol_) + " illegal", __FILE__, __LINE__));
            terminate(istate);
            return;
        }
        if (iopt < 0 || iopt > 1)
        {
            errors.push_back(Errors("[lsoda] iopt = " + to_string(iopt) + " illegal", __FILE__, __LINE__));
            terminate(istate);
            return;
        }
        if (jt == 3 || jt < 1 || jt > 5)
        {
            errors.push_back(Errors("[lsoda] jt = " + to_string(jt) + " illegal", __FILE__, __LINE__));
            terminate(istate);
            return;
        }
        jtyp = jt;
        if (jt > 2)
        {
            ...
            if (ml >= n)
            {
                errors.push_back(Errors("[lsoda] ml = " + to_string(ml) + " not between 1 and neq", __FILE__, __LINE__));
                terminate(istate);
                return;
            }
            if (mu >= n)
            {
                errors.push_back(Errors("[lsoda] mu = " + to_string(mu) + " not between 1 and neq", __FILE__, __LINE__));
                terminate(istate);
                return;
            }
        }
        ...
        else /* if ( iopt = 1 )  */
        {
            ...
            if (ixpr > 1)
            {
                errors.push_back(Errors("[lsoda] ixpr = " + to_string(ixpr) + " is illegal", __FILE__, __LINE__));
                terminate(istate);
                return;
            }
            ...
            if (*istate == 1)
            {
                ...
                if ((tout - *t) * h0 < 0.)
                {
                    errors.push_back(Errors("[lsoda] tout = " + to_string(tout) + " behind t = " + to_string(*t) + ". integration direction is given by " + to_string(h0), __FILE__, __LINE__));
                    terminate(istate);
                    return;
                }
            } /* end if ( *istate == 1 )  */
            ...
            if (hmax < 0.)
            {
                errors.push_back(Errors("[lsoda] hmax < 0.", __FILE__, __LINE__));
                terminate(istate);
                return;
            }
            hmxi = 0.;
            ...
            if (hmin < 0.)
            {
                errors.push_back(Errors("[lsoda] hmin < 0.", __FILE__, __LINE__));
                terminate(istate);
                return;
            }
        } /* end else   */ /* end iopt = 1   */
    ...
    */
    if (*istate == 1 || *istate == 3)
    {
        ...
        for (size_t i = 1; i <= n; i++)
        {
            ...
            if (rtoli < 0.)
            {
                errors.push_back(Errors("[lsoda] rtol = " + to_string(rtoli) + " is less than 0.", __FILE__, __LINE__));
                terminate(istate);
                return;
            }
            if (atoli < 0.)
            {
                errors.push_back(Errors("[lsoda] atol = " + to_string(atoli) + " is less than 0.", __FILE__, __LINE__));
                terminate(istate);
                return;
            }
        } /* end for   */
    ...
    */
    if (*istate == 1)
    {
        ...
        if (itask == 4 || itask == 5)
        {
            ...
            {
                errors.push_back(Errors("[lsoda] itask = 4 or 5 and tcrit behind tout", __FILE__, __LINE__));
                terminate(istate);
                return;
            }
        ...
        for (size_t i = 1; i <= n; i++)
        {
            if (ewt[i] <= 0.)
            {
                warnings.push_back(Errors("[lsoda] ewt[" + to_string(i) + "] = " + to_string(ewt[i]) + " <= 0.", __FILE__, __LINE__));
                terminate2(y, t);
                return;
            }
            ewt[i] = 1. / ewt[i];
        ...
        */
        if (h0 == 0.)
        {
            ...
            if (tdist < 2. * ETA * w0)
            {
                errors.push_back(Errors("[lsoda] tout too close to t to start integration", __FILE__, __LINE__));
                terminate(istate);
                return;
            }
            tol = rtol_[1];
    ...
    if (*istate == 2 || *istate == 3)
    {
        ...
        case 1:
            if ((tn_ - tout) * h_ >= 0.)
            {
                ...
                if (iflag != 0)
                {
                    errors.push_back(Errors("[lsoda] trouble from intdy, itask = " + to_string(itask) + ", tout = " + to_string(tout), __FILE__, __LINE__));
                    terminate(istate);
                    return;
                }
                *t = tout;
        ...
        case 3:
            ...
            if ((tp - tout) * h_ > 0.)
            {
                errors.push_back(Errors("[lsoda] itask = " + to_string(itask) + " and tout behind tcur - hu", __FILE__, __LINE__));
                terminate(istate);
                return;
            }
            if ((tn_ - tout) * h_ < 0.)
        ...
        case 4:
            ...
            if ((tn_ - tcrit) * h_ > 0.)
            {
                errors.push_back(Errors("[lsoda] itask = 4 or 5 and tcrit behind tcur", __FILE__, __LINE__));
                terminate(istate);
                return;
            }
            if ((tcrit - tout) * h_ < 0.)
    ...
    */
    while (1)
    {
        if (*istate != 1 || nst != 0)
        {
            ...
            {
                warnings.push_back(Errors("[lsoda] " + to_string(mxstep) + " steps taken before reaching tout", __FILE__, __LINE__));
                *istate = -1;
                terminate2(y, t);
                return;
            }
        ...
        if (tolsf > 0.01)
        {
            ...
            if (nst == 0)
            {
                errors.push_back(Errors("lsoda -- at start of problem, too much accuracy requested for precision of machine, suggested scaling factor = " + to_string(tolsf), __FILE__, __LINE__));
                terminate(istate);
                return;
            }
            errors.push_back(Errors("lsoda -- at t = %g, too much accuracy requested for precision of machine, suggested scaling factor = " + to_string(tolsf), __FILE__, __LINE__));
            *istate = -2;
            terminate2(y, t);
            return;
        }
        ...
        if ((tn_ + h_) == tn_)
        {
            ...
            if (nhnil <= mxhnil)
            {
                warnings.push_back(Errors("lsoda -- warning..internal t = " + to_string(tn_) + " and h_ = " + to_string(h_) + " are such that in the machine, t + h_ = t on the next step solver will continue anyway.", __FILE__, __LINE__));
                if (nhnil == mxhnil)
                {
                    warnings.push_back(Errors("lsoda -- above warning has been issued " + to_string(nhnil) + " times. it will not be issued again for this problem", __FILE__, __LINE__));
                }
            }
        }
        ...
        if (kflag == 0)
        {
            ...
            */
            ...
            if (meth_ != mused)
            {
                ...
                {
                    if (meth_ == 2)
                        warnings.push_back(Errors("[lsoda] a switch to the stiff method has occurred", __FILE__, __LINE__));
                    if (meth_ == 1)
                        warnings.push_back(Errors("[lsoda] a switch to the nonstiff method has occurred", __FILE__, __LINE__));
                }
            } /* end if ( meth_ != mused )   */
            /*
               itask = 1.
        ...
        */
        if (kflag == -1 || kflag == -2)
        {
            ...
            warningMessage << "lsoda -- at t = " << to_string(tn_) << " and step size h_ = " << to_string(h_) << ", the";
            if (kflag == -1)
            {
                warningMessage << " error test failed repeatedly or with fabs(h_) = hmin";
                *istate = -4;
            }
            if (kflag == -2)
            {
                warningMessage << " corrector convergence failed repeatedly or with fabs(h_) = hmin";
                *istate = -5;
            }
...
void LSODA::intdy(double t, int k, vector<double> &dky, int *iflag)
{
    ...
    *iflag = 0;
    if (k < 0 || k > (int)nq)
    {
        warnings.push_back(Errors("[intdy] k = " + to_string(k) + " illegal", __FILE__, __LINE__));
        *iflag = -1;
        return;
    }
    ...
    if ((t - tp) * (t - tn_) > 0.)
    {
        warnings.push_back(Errors("intdy -- t = " + to_string(t) + " illegal. t not in interval tcur - hu to tcur", __FILE__, __LINE__));
        *iflag = -2;
        return;
    }
    s = (t - tn_) / h_;
                 ...
                 void *_data)
{
    ...
    */
    if (miter != 2)
    {
        warnings.push_back(Errors("[prja] miter != 2", __FILE__, __LINE__));
        return;
    }
    if (miter == 2)
    {
                        ...
                        )
{
    ...
    lsoda(f, neq, y, t, tout, itask, istate, iopt, jt, iworks, rworks, _data);
}
