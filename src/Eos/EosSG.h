...
class EosSG : public Eos
{
  public:
    ...
    //! \param     pressure           pressure (p)
    //! \return    soundSpeed
    //! \details   with soundSpeed : \f$  c(p, \rho)  = \sqrt{  \frac{(\gamma-1) \ p_I + p + \gamma \ p_{\infty}}{\rho}} \f$
    ...
    //! \details   with : \f$  z(p, \rho)  = \sqrt{ \rho \ \gamma \ (p+p_{\infty}) } \f$
    ...
    //! \param     pressure           pressure (p)
    //! \return    rhoCISquare
    //! \details   with : \f$  z(p, \rho)  = (\gamma-1) \ p_I + p + \gamma \ p_{\infty} \f$
    double
    ...
    //! \return  \f$ \ "SG" \f$
    TypeEOS getType() const override { return TypeEOS::SG; };
